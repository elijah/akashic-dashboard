...
exports.WorldClockPanel = exports.MacroSignalsPanel = exports.FearGreedPanel = exports.CryptoPanel = exports.StrategicPosturePanel = exports.CountryInstabilityPanel = exports.CrossSourceSignal = exports.ThreatTimeline = exports.StrategicRiskOverview = exports.LiveIntelligenceFeed = exports.live_panels = exports.LayerSignalPanel = exports.layer_signal_panels = exports.LiveStreamsPanel = exports.LatestFlightsPanel = exports.SuperpowerTrackerPanel = exports.ContinentNewsGrid = exports.EnergyResourcesPanel = exports.GlobalStockMarketsPanel = void 0;
const react_1 = require("react");
const live_deck_core_1 = require("@/lib/geo-intelligence/live-deck-core");
const MarketDeepDive_1 = require("@/components/popups/MarketDeepDive");
const IntelNewsReader_1 = require("@/components/popups/IntelNewsReader");
const FlightMap_1 = require("@/components/popups/FlightMap");
const LiveStreamDeck_1 = require("@/components/popups/LiveStreamDeck");
...
const Block = ({ title, children, extra, extraHead, onClick }) => (<div onClick={onClick} className={`flex flex-col bg-[#0f0f0f] border border-[#222] rounded-md overflow-hidden ${onClick ? 'cursor-pointer hover:border-stone-500 transition-colors' : ''} ${extra || ''}`}>
...
const GlobalStockMarketsPanel = ({ data }) => {
    const [activeSymbol, setActive] = (0, react_1.useState)(null);
    return (<>
      <Block title="global stock markets" extra="col-span-1 lg:col-span-2 row-span-2" extraHead={<span className="text-[9px] bg-green-500/20 text-green-500 px-1.5 rounded">LIVE TICKER</span>}>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
          {data.markets?.map(m => (<div key={m.id} onClick={() => setActive(m.symbol)} className="cursor-pointer bg-[#111] border border-[#222] p-2 flex flex-col hover:border-stone-500 transition-colors">
      ...
      {activeSymbol && <MarketDeepDive_1.MarketDeepDive symbol={activeSymbol} onClose={() => setActive(null)}/>}
    </>);
};
exports.GlobalStockMarketsPanel = GlobalStockMarketsPanel;
const EnergyResourcesPanel = ({ data }) => {
    const [activeSymbol, setActive] = (0, react_1.useState)(null);
    return (<>
      <Block title="energy & resources price" extra="row-span-2" extraHead={<span className="text-[9px] bg-yellow-500/20 text-yellow-500 px-1.5 rounded">COMMODITIES</span>}>
        <div className="grid grid-cols-2 gap-2">
          {data.energy_prices?.map(m => (<div key={m.id} onClick={() => setActive(m.symbol)} className="cursor-pointer bg-[#111] border border-[#222] p-2 flex flex-col hover:border-yellow-500/50 transition-colors">
              <div className="flex justify-between items-center">
...
exports.EnergyResourcesPanel = EnergyResourcesPanel;
const ContinentNewsGrid = ({ data }) => {
    const [activeNews, setActiveNews] = (0, react_1.useState)(null);
    return (<>
      <Block title="7 continent major latest news" extra="col-span-1 md:col-span-2 row-span-3" extraHead={<span className="text-[9px] border border-[#0ea5e9] text-[#0ea5e9] px-1.5 rounded">GLOBAL GRID</span>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 h-full">
          {data.continent_news?.map(c => (<div key={c.continent} onClick={() => setActiveNews({ title: c.continent, events: c.events })} className="cursor-pointer flex flex-col gap-1 p-2 bg-[#111] border border-[#222] rounded hover:border-[#0ea5e9]/50 transition-colors">
              ...
              {c.events.slice(0, 2).map(e => (<div key={e.id} className="flex flex-col mb-1 last:mb-0">
      ...
      {activeNews && <IntelNewsReader_1.IntelNewsReader title={activeNews.title} events={activeNews.events} onClose={() => setActiveNews(null)}/>}
    </>);
};
exports.ContinentNewsGrid = ContinentNewsGrid;
const SuperpowerTrackerPanel = ({ data }) => {
    const [activeNews, setActiveNews] = (0, react_1.useState)(null);
    return (<>
      <Block title="superpower latest news" extra="row-span-3" extraHead={<span className="text-[9px] bg-red-500/20 text-red-500 px-1.5 rounded">GEO-POL</span>}>
        <div className="flex flex-col gap-2 h-full">
          {data.superpower_news?.map(s => (<div key={s.entity} onClick={() => setActiveNews({ title: s.entity, events: s.events })} className="cursor-pointer flex flex-col p-2 bg-[#111] border-l-2 border-l-[#ff6b00] rounded hover:bg-[#1a110a] transition-colors">
              ...
              {s.events.slice(0, 1).map(e => (<div key={e.id} className="flex flex-col">
...
exports.SuperpowerTrackerPanel = SuperpowerTrackerPanel;
const LatestFlightsPanel = ({ data }) => {
      ...
      <Block title="latest flights & airspace" extra="row-span-2 cursor-pointer hover:border-blue-500 transition-colors" onClick={() => setOpen(true)} extraHead={<span className="text-[9px] border border-blue-500 text-blue-500 px-1.5 rounded">RADAR</span>}>
        <div className="flex flex-col gap-2 h-full">
          ...
          <div className="flex flex-col gap-1 flex-1 overflow-y-auto">
            {aviationEvents.length > 0 ? aviationEvents.slice(0, 5).map(e => (<div key={e.id} className="flex flex-col bg-[#1a1a1a] p-1.5 rounded">
              ...
              </div>)) : (<div className="text-[10px] text-stone-500 text-center py-2">No severe flight anomalies tracked.</div>)}
          </div>
        </div>
      </Block>
      {open && <FlightMap_1.FlightMap events={aviationEvents} onClose={() => setOpen(false)}/>}
    </>);
};
exports.LatestFlightsPanel = LatestFlightsPanel;
const LiveStreamsPanel = ({ data }) => {
    const [open, setOpen] = (0, react_1.useState)(false);
    return (<>
      ...
      {open && <LiveStreamDeck_1.LiveStreamDeck onClose={() => setOpen(false)}/>}
    </>);
};
exports.LiveStreamsPanel = LiveStreamsPanel;
const tonez = {
...
const domain_panel = ({ data, id }) => {
    const group = (0, live_deck_core_1.build_live_deck)(data).groups.find(item => item.id === id);
    if (!group)
        return null;
    const tone = tonez[group.color] || tonez.stone;
    return (<Block title={group.title} extraHead={<span className={`text-[9px] border px-1.5 rounded uppercase ${tone.text} ${tone.border}`}>live {group.count}</span>}>
      <div className="flex flex-col gap-2 min-h-[184px]">
        ...
        <div className="flex flex-wrap gap-1">
          {group.layers.map(layer => (<span key={layer} className="text-[8px] text-stone-400 border border-[#2c2c2c] bg-[#111] px-1.5 py-0.5 rounded">
              {layer.replaceAll("_", " ")} {data.counts?.[layer] || 0}
            </span>))}
        </div>
        <div className="flex flex-col gap-1">
          {group.events.length ? group.events.slice(0, 4).map(event => (<div key={event.id} className="flex gap-2 border-t border-[#222] pt-1.5 first:border-0 first:pt-0 min-w-0">
            ...
            </div>)) : (<span className="text-[9px] text-stone-500 py-2">reference layers active; awaiting live event matches</span>)}
        </div>
      </div>
    </Block>);
};
const source_health_panel = ({ data }) => {
    const health = data.source_health;
    const total = health.active_sources + health.delayed_sources + health.stale_sources;
    return (<Block title="source health" extraHead={<span className="text-[9px] border border-green-500/40 text-green-400 px-1.5 rounded">live {health.active_sources}</span>}>
      <div className="flex flex-col gap-2 min-h-[150px]">
        ...
        ].map(([label, val, color]) => (<div key={label} className="bg-[#111] border border-[#222] p-2 flex flex-col">
              <span className={`text-lg font-bold ${color}`}>{val}</span>
              <span className="text-[8px] text-stone-500 uppercase">{label}</span>
            </div>))}
        </div>
        <div className="flex flex-col gap-1">
          {health.source_groups.map(source => (<div key={source.name} className="flex items-center justify-between border-t border-[#222] pt-1.5 first:border-0">
...
const live_correlations_panel = ({ data }) => {
    const correlations = data.correlations.slice(0, 5);
    return (<Block title="live correlations" extraHead={<span className="text-[9px] border border-cyan-500/40 text-cyan-400 px-1.5 rounded">{correlations.length} linked</span>}>
      <div className="flex flex-col gap-2 min-h-[184px]">
        {correlations.length ? correlations.map(item => (<div key={item.id} className="border-l-2 border-cyan-500/60 bg-[#111] px-2 py-1.5">
          ...
          </div>)) : <span className="text-[9px] text-stone-500 py-3 text-center">no verified cross-domain correlation in the current window</span>}
      </div>
    </Block>);
};
...
const LayerSignalPanel = ({ data, id, title, badge, tone = "stone" }) => {
    ...
    return (<Block title={title} extra="min-h-[210px]" extraHead={<span className={`text-[9px] border px-1.5 rounded uppercase ${cfg.text} ${cfg.border}`}>{badge}</span>}>
      <div className="flex h-full flex-col gap-2">
        ...
        <div className="flex flex-1 flex-col gap-1">
          {events.length ? events.map(event => (<div key={event.id} className="border-t border-[#222] pt-1 first:border-0 first:pt-0">
            ...
            </div>)) : (<span className="text-[9px] text-stone-500">registry layer ready; no live event rows in the current feed window</span>)}
        </div>
      </div>
    </Block>);
};
exports.LayerSignalPanel = LayerSignalPanel;
const worldmonitor_panels_panel = ({ data }) => {
    const counts = data.counts || {};
    const total_layers = wm_panel_groups.reduce((sum, group) => sum + group.layers.filter(layer => counts[layer] > 0).length, 0);
    return (<Block title="worldmonitor panel matrix" extraHead={<span className="text-[9px] border border-sky-500/40 text-sky-400 px-1.5 rounded">{total_layers} live layers</span>}>
      <div className="flex flex-col gap-2 min-h-[220px]">
        {wm_panel_groups.map(group => {
            const signals = group.layers.reduce((sum, layer) => sum + (counts[layer] || 0), 0);
            return (<div key={group.id} className={`rounded border px-2 py-1.5 ${group.tone}`}>
              <div className="flex items-center justify-between gap-2">
              ...
              <div className="mt-1 flex flex-wrap gap-1">
                {group.panels.map(panel => (<span key={panel} className="rounded border border-white/10 bg-black/20 px-1.5 py-0.5 text-[7px] uppercase text-stone-300">
                    {panel}
                  </span>))}
              </div>
              <div className="mt-1 flex flex-wrap gap-1">
                {group.layers.filter(layer => counts[layer] > 0).slice(0, 8).map(layer => (<span key={layer} className="text-[7px] uppercase text-stone-500">
                    {layer.replaceAll("_", " ")} {counts[layer]}
                  </span>))}
              </div>
            </div>);
        })}
...
exports.live_panels = {
    conflict_humanitarian: (props) => domain_panel({ ...props, id: "conflict_humanitarian" }),
    infrastructure_flow: (props) => domain_panel({ ...props, id: "infrastructure_flow" }),
    cyber_connectivity: (props) => domain_panel({ ...props, id: "cyber_connectivity" }),
    strategic_assets: (props) => domain_panel({ ...props, id: "strategic_assets" }),
    natural_climate: (props) => domain_panel({ ...props, id: "natural_climate" }),
    sanctions_economic: (props) => domain_panel({ ...props, id: "sanctions_economic" }),
    technology_watch: (props) => domain_panel({ ...props, id: "technology_watch" }),
    resilience_progress: (props) => domain_panel({ ...props, id: "resilience_progress" }),
    intelligence_hotspots: (props) => domain_panel({ ...props, id: "intelligence_hotspots" }),
    worldmonitor_panels: worldmonitor_panels_panel,
    source_health: source_health_panel,
    live_correlations: live_correlations_panel,
};
const LiveIntelligenceFeed = ({ data }) => {
    const events = (0, live_deck_core_1.build_live_deck)(data).events;
    return (<>
      <Block title="live intelligence" extra="row-span-2 h-full">
        ...
        <div className="flex flex-col gap-2 flex-1 overflow-y-auto mt-2 pr-1">
          {events.length > 0 ? events.map(e => (<div key={e.id} className="flex gap-2 text-[11px] p-2 bg-[#111] hover:bg-[#1a1a1a] border border-[#222] rounded group transition-colors">
            ...
            </div>)) : (<div className="text-[10px] text-stone-500 text-center py-4 uppercase">Waiting for signals...</div>)}
        </div>
      </Block>
    </>);
};
exports.LiveIntelligenceFeed = LiveIntelligenceFeed;
const StrategicRiskOverview = ({ data }) => {
    ...
    return (<Block title="strategic risk overview" extraHead={<span className="text-[9px] border border-[#eab308] text-[#eab308] px-1.5 rounded">LIVE · {totalCount} events</span>}>
...
exports.StrategicRiskOverview = StrategicRiskOverview;
const ThreatTimeline = ({ data }) => {
    ...
    return (<Block title="threat timeline" extraHead={<span className="text-[9px] border border-green-500 text-green-500 px-1.5 rounded">LIVE EVENTS</span>}>
      <div className="flex flex-col h-full justify-between">
        ...
        <div className="flex items-end justify-between mt-4 h-16 px-2">
          {dayEntries.map(([dateStr, stats], idx) => {
            ...
            return (<div key={dateStr} className="flex flex-col items-center gap-0 w-6">
                {stats.total > 0 ? (<>
...
exports.ThreatTimeline = ThreatTimeline;
const CrossSourceSignal = ({ data }) => {
    const categories = {};
    data.events.forEach(e => {
        if (!categories[e.category])
            categories[e.category] = { critical: 0, high: 0, med: 0, events: [] };
        categories[e.category].events.push(e);
        if (e.severity === 'critical')
            categories[e.category].critical++;
        else if (e.severity === 'high')
            categories[e.category].high++;
        else
            categories[e.category].med++;
    });
    const topSig = Object.entries(categories).sort((a, b) => (b[1].critical * 2 + b[1].high) - (a[1].critical * 2 + a[1].high))[0];
    return (<Block title="cross-source signal aggregator">
...
exports.CrossSourceSignal = CrossSourceSignal;
const CountryInstabilityPanel = ({ data }) => {
    const scores = {};
    data.events.forEach(e => {
    ...
    return (<Block title="country instability" extraHead={<span className="text-[9px] border border-[#ff6b00] text-[#ff6b00] px-1.5 rounded">LIVE</span>}>
      <div className="flex flex-col gap-3">
        {list.length > 0 ? list.map(c => (<div key={c.name} className="flex flex-col gap-1 border-b border-[#222] pb-2 last:border-0">
            <div className="flex justify-between items-center">
          ...
          </div>)) : (<div className="text-stone-500 text-[10px] text-center mt-4">No regional instability detected.</div>)}
      </div>
    </Block>);
};
exports.CountryInstabilityPanel = CountryInstabilityPanel;
const StrategicPosturePanel = ({ data }) => {
    const theaters = {
    ...
    return (<Block title="strategic posture" extraHead={<span className="text-[9px] bg-white/10 px-1.5 rounded text-white">{(0, live_deck_core_1.build_live_deck)(data).events.length} SIGNALS</span>}>
      <div className="flex flex-col gap-2">
        {sortedTheaters.length > 0 ? sortedTheaters.map(([name, t]) => {
            const isElev = t.air + t.sea > 2;
            return (<div key={name} className={`p-2 border rounded flex flex-col gap-2 ${isElev ? 'border-yellow-500/20 bg-yellow-500/10' : 'border-green-500/20 bg-green-500/5'}`}>
              <div className="flex justify-between items-center">
...
const CryptoPanel = ({ data }) => {
    return (<Block title="crypto prices" extraHead={<span className="text-[9px] border border-blue-500 text-blue-500 px-1.5 rounded">DIGITAL</span>}>
      <div className="flex flex-col gap-2">
        {data.crypto?.map(c => (<div key={c.id} className="flex items-center justify-between border-b border-[#222] pb-1 last:border-0 last:pb-0">
            <div className="flex flex-col">
...
const FearGreedPanel = ({ data }) => {
    const fg = data.fear_greed;
    if (!fg)
        return null;
    return (<Block title="fear & greed index" extraHead={<span className="text-[9px] bg-[#222] text-stone-300 px-1.5 rounded">SENTIMENT</span>}>
...
exports.FearGreedPanel = FearGreedPanel;
const MacroSignalsPanel = ({ data }) => {
    return (<Block title="macro signals" extraHead={<span className="text-[9px] bg-purple-500/20 text-purple-500 px-1.5 rounded">ECONOMY</span>}>
      <div className="flex flex-col gap-2">
        {data.macro_signals?.map(m => (<div key={m.label} className="flex justify-between items-center bg-[#111] border border-[#222] p-2 hover:border-purple-500/50 transition-colors">
...
const WorldClockPanel = () => {
    ...
    return (<Block title="world clock" extraHead={<span className="text-[9px] text-stone-500 border border-stone-700 px-1.5 rounded">TIME</span>}>
      <div className="grid grid-cols-2 gap-2 h-full">
        {cities.map(c => {
            const str = time.toLocaleTimeString("en-US", { timeZone: c.tz, hour: '2-digit', minute: '2-digit', hour12: false });
            return (<div key={c.name} className="flex flex-col bg-[#111] border border-[#222] p-2 items-center justify-center">
...
exports.WorldClockPanel = WorldClockPanel;
