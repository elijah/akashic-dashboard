...
const StreetViewMap_1 = require("./StreetViewMap");
const map_core_1 = require("@/lib/weather/map-core");
...
const wm_color = (id, a = 220) => {
    if (id === "conflicts" || id === "wars" || id === "iran_attacks")
        return [239, 68, 68, a];
    if (id === "civil_unrest")
        return [245, 158, 11, a];
    if (id === "violence" || id === "wildfires" || id === "natural_events" || id === "climate_anomalies")
        return [249, 115, 22, a];
    if (id === "humanitarian" || id === "internet_disruptions")
        return [6, 182, 212, a];
    if (id === "nuclear" || id === "radiation_watch")
        return [217, 70, 239, a];
    if (id === "irradiators")
        return [250, 204, 21, a];
    if (id === "bases")
    ...
    if (id === "sanctions")
        return [220, 38, 38, a];
    if (id === "spaceports" || id === "orbital_surveillance")
        return [168, 85, 247, a];
    if (id === "chokepoints" || id === "gps_jamming")
        return [59, 130, 246, a];
    if (id === "trade_routes" || id === "live_tankers" || id === "commodity_ports")
        return [56, 189, 248, a];
    if (id === "critical_minerals" || id === "cyber_threats" || id === "mining_sites" || id === "processing_plants")
        return [99, 102, 241, a];
    if (id === "economic_centers" || id === "central_banks" || id === "happiness")
        return [234, 179, 8, a];
    if (id === "stock_exchanges" || id === "financial_centers" || id === "positive_events" || id === "kindness" || id === "species_recovery" || id === "renewable_installations" || id === "resilience_score")
        return [34, 197, 94, a];
    if (id === "data_centers" || id === "startup_hubs" || id === "cloud_regions" || id === "accelerators" || id === "tech_hqs" || id === "tech_events")
        return [34, 211, 238, a];
    if (id === "disease_outbreaks")
        return [132, 204, 22, a];
    if (id === "storage_facilities")
        return [120, 113, 108, a];
    return [34, 211, 238, a];
};
const wm_radius = (id, sev) => {
...
const RADAR_REFRESH_MS = 60000;
const USGS_REFRESH_MS = 5 * 60000;
const FLIGHT_STALE_AFTER_MS = 4 * 60000;
const SATELLITE_CATALOG_REFRESH_MS = 30 * 60000;
const SATELLITE_POSITION_REFRESH_MS = 20000;
const WEATHER_REFRESH_MS = 10 * 60000;
const RADIO_REFRESH_MS = 15 * 60000;
const SIGINT_REFRESH_MS = 30000;
const GDACS_REFRESH_MS = 15 * 60000;
const AIRQ_REFRESH_MS = 30 * 60000;
const PATH_MAX_POINTS = 80;
const PATH_RECORD_INTERVAL_MS = 5000;
const sigint_severity_rank = { low: 0, medium: 1, high: 2, critical: 3 };
const sigint_color = (severity, alpha) => severity === "critical" ? [248, 113, 113, alpha] :
    severity === "high" ? [251, 146, 60, alpha] :
        severity === "medium" ? [250, 204, 21, alpha] :
            [45, 212, 191, alpha];
const event_color = (severity, alpha) => severity === "critical" ? [239, 68, 68, alpha] :
    severity === "high" ? [249, 115, 22, alpha] :
        severity === "elevated" ? [250, 204, 21, alpha] :
            [45, 212, 191, alpha];
const airq_color = (aqi, alpha) => aqi === null ? [148, 163, 184, alpha] :
    aqi >= 201 ? [239, 68, 68, alpha] :
        aqi >= 151 ? [249, 115, 22, alpha] :
            aqi >= 101 ? [250, 204, 21, alpha] :
                [34, 197, 94, alpha];
function mkSvg(c, s = 96) {
...
function planeIcon(fill, stroke, path, glow = true) {
...
const ICON_WIDE_BODY = { fill: "#000000", stroke: "#444444", path: P_WIDE };
const ICON_AIRLINER = { fill: "#000000", stroke: "#3a3a3a", path: P_AIRLINER };
const ICON_REGIONAL = { fill: "#111111", stroke: "#555555", path: P_REGIONAL };
const ICON_BIZJET = { fill: "#78350f", stroke: "#fde68a", path: P_BIZJET };
const ICON_GA = { fill: "#14532d", stroke: "#4ade80", path: P_GA };
const ICON_HELI = { fill: "#831843", stroke: "#f9a8d4", path: P_HELI };
const ICON_MIL_FIGHTER = { fill: "#ef4444", stroke: "#fca5a5", path: P_FIGHTER };
const ICON_MIL_TRANSPORT = { fill: "#ea580c", stroke: "#fed7aa", path: P_TRANSPORT };
const ICON_MIL_TANKER = { fill: "#b45309", stroke: "#fde68a", path: P_TANKER };
const ICON_LADD = { fill: "#4c1d95", stroke: "#c4b5fd", path: P_BIZJET };
const ICON_PIA = { fill: "#581c87", stroke: "#e879f9", path: P_REGIONAL };
const ICON_DRONE = { fill: "#0e7490", stroke: "#67e8f9", path: P_DRONE };
const ICON_SEAPLANE = { fill: "#075985", stroke: "#7dd3fc", path: P_SEAPLANE };
const ICON_TILTROTOR = { fill: "#78350f", stroke: "#fbbf24", path: P_TILT };
const ICON_GROUND = { fill: "#1c1c1c", stroke: "#555555", path: P_AIRLINER };
function iconForType(t, category, onGround) {
    if (onGround)
        return ICON_GROUND;
    if (category === "MILITARY") {
        ...
        if (/^(F1[56789]|F2[02]|F3[56]|EF[79][0-9]|MIG|SU[0-9]|JA3[79]|B2|F22|F35|FA18|F16)/.test(tu))
            return ICON_MIL_FIGHTER;
        if (/^(KC|E[38]|P[38]|EP|RC|AWAC)/.test(tu))
            return ICON_MIL_TANKER;
        return ICON_MIL_TRANSPORT;
    }
    if (category === "LADD")
        return ICON_LADD;
    ...
    const tu = t.toUpperCase();
    if (/^(B74[47]|B77[237W]|B78[89]|A3[34][023]|A34[05]|A35[09]|A38[08])/.test(tu))
        return ICON_WIDE_BODY;
    if (/^(B7[23][678]|B737|B738|B739|A31[89]|A32[01NEP]|A220|A32[12]|E19[05])/.test(tu))
        return ICON_AIRLINER;
    if (/^(CONC|TU1[34][46])/.test(tu))
        return ICON_AIRLINER;
    if (/^(E1[37][05]|CRJ|E7[05]|DH8|AT[57][23]|SF34|F50|F7[02])/.test(tu))
        return ICON_REGIONAL;
    if (/^(GLF|C56|C68|FA[57][0-9]|HA4T|CL[36]|GLEX|G[56][0-9][0-9]|BE40|C25|PC24|BE[19])/.test(tu))
        return ICON_BIZJET;
    if (/^(EC[0-9]|H[16][025]|S[076][0-9]|B06|B07|R22|R44|AW[139]|UH1|CH[45])/.test(tu))
        return ICON_HELI;
    if (/^(GLOBA|RQ[0-9]|MQ[0-9]|PRED|REAP|UAV)/.test(tu))
        return ICON_DRONE;
    if (/^(DHC[23]|C2[358]|BN2)/.test(tu))
        return ICON_SEAPLANE;
    if (/^(V22|MV22|BA609|V280)/.test(tu))
        return ICON_TILTROTOR;
    if (/^(C1[72][2568]|C1[89][0-9]|PA2[48]|PA3[46]|DA4[02]|SR2[02]|TB2|P28)/.test(tu))
        return ICON_GA;
    return ICON_AIRLINER;
}
const iconCache = new Map();
function resolveIcon(t, category, onGround) {
...
function classifySatellite(name) {
    ...
    if (u.includes("ISS") || u.includes("ZARYA") || u.includes("TIANGONG") || u.includes("TIANHE") || u.includes("WENTIAN") || u.includes("MENGTIAN"))
        return { category: "space-station", operator: u.includes("ISS") || u.includes("ZARYA") ? "international consortium" : "CNSA", purpose: "crewed space station" };
    if (u.match(/^GPS\b/) || u.includes("NAVSTAR"))
        return { category: "navigation", operator: "US Space Force", purpose: "global positioning system" };
    if (u.includes("GLONASS"))
        return { category: "navigation", operator: "Roscosmos", purpose: "global navigation system" };
    if (u.includes("GALILEO"))
        return { category: "navigation", operator: "ESA", purpose: "global navigation system" };
    if (u.includes("BEIDOU") || u.includes("COMPASS"))
        return { category: "navigation", operator: "CNSA", purpose: "chinese navigation system" };
    if (u.includes("NOAA") || u.includes("GOES") || u.includes("METEOSAT") || u.includes("METEOR-") || u.includes("HIMAWARI") || u.includes("FY-") || u.includes("FENGYUN"))
        return { category: "weather", operator: u.includes("NOAA") || u.includes("GOES") ? "NOAA" : u.includes("METEOSAT") ? "EUMETSAT" : u.includes("HIMAWARI") ? "JMA" : "CNSA", purpose: "meteorological observation" };
    if (u.includes("STARLINK"))
        return { category: "comms", operator: "SpaceX", purpose: "broadband internet constellation" };
    if (u.includes("ONEWEB"))
        return { category: "comms", operator: "OneWeb", purpose: "broadband internet constellation" };
    if (u.includes("INTELSAT"))
        return { category: "comms", operator: "Intelsat", purpose: "communications satellite" };
    if (u.includes("SES-") || u.includes("ASTRA"))
        return { category: "comms", operator: "SES", purpose: "communications satellite" };
    if (u.includes("IRIDIUM"))
        return { category: "comms", operator: "Iridium", purpose: "mobile satellite communications" };
    if (u.includes("ORBCOMM"))
        return { category: "comms", operator: "ORBCOMM", purpose: "IoT/M2M data communications" };
    if (u.includes("LANDSAT") || u.includes("SENTINEL") || u.includes("SPOT ") || u.includes("TERRA") || u.includes("AQUA") || u.includes("WORLDVIEW") || u.includes("GEOEYE") || u.includes("PLEIADES"))
        return { category: "earth-obs", operator: u.includes("SENTINEL") ? "ESA" : u.includes("WORLDVIEW") || u.includes("GEOEYE") || u.includes("PLEIADES") ? "commercial" : "civil remote sensing", purpose: "earth observation / remote sensing" };
    if (u.includes("HUBBLE") || u.includes("HST"))
        return { category: "science", operator: "space telescope consortium", purpose: "space telescope" };
    if (u.includes("CHANDRA"))
        return { category: "science", operator: "science mission", purpose: "x-ray observatory" };
    if (u.includes("USA-") || u.includes("NROL") || u.includes("LACROSSE") || u.includes("KH-"))
        return { category: "military", operator: "NRO/US DoD", purpose: "classified reconnaissance" };
    if (u.includes("COSMOS") && !u.includes("COSMOSKY"))
        return { category: "military", operator: "Russian MoD", purpose: "classified military" };
    if (u.includes(" DEB") || u.includes("DEB)") || u.includes("R/B") || u.includes("ROCKET BODY") || u.includes("BOOSTER"))
        return { category: "debris", operator: null, purpose: "orbital debris / rocket body" };
    return { category: "unknown", operator: null, purpose: null };
}
function satPos(sr, t) {
...
function destPoint(lat, lon, brngRad, dMeters) {
    const R = 6371e3;
...
function projectedFlightPositionAt(flight, atMs) {
...
function collectLngLatPairs(value, out = []) {
...
function buildingFeatureCenter(feature, fallback) {
...
function buildingFeatureName(feature) {
    const props = feature?.properties || {};
    return props["name:en"] || props.name || props.ref || props.type || undefined;
}
function gilTierFor(city) {
    if (city.pop >= 10000000)
        return 5;
    if (city.pop >= 1000000)
        return 4;
    if (city.pop >= 500000 || city.capital)
        return 3;
    if (city.pop >= 100000)
        return 2;
    return 1;
}
function toNum(v) { if (typeof v === "number" && Number.isFinite(v))
    return v; if (typeof v === "string" && v.trim() !== "") {
    const p = Number(v);
...
function MapGlobe({ activeStyle, activeImagery, isGlobe, streetViewMode, enabledLayers, radarFilter = "all", reconLocations = [], weatherMode = "wind", sigintFilters, uiCommand, onFlightSelect, onAirportSelect, onSatelliteSelect, onQuakeSelect, onRadioSelect, onCameraSelect, onIntelSelect, onSigintSelect, onSigintData, onCitySelect, onCountrySelect, onRadarCount, onUsgsCount, onRadioCount, onSigintCount, onGdacsCount, onAirqCount, onGilCount, onPositionFix, onViewMetrics, onStreetViewPick, worldMonitorLayers, onWmCounts, onBuildingSelect }) {
    const [streetViewPos, setStreetViewPos] = (0, react_1.useState)({ lat: 0, lon: 0, zoom: 13 });
    const [snapshot, setSnapshot] = (0, react_1.useState)(null);
    const [selectedPano, setSelectedPano] = (0, react_1.useState)(null);
    const containerRef = (0, react_1.useRef)(null);
    const mapRef = (0, react_1.useRef)(null);
    const deckRef = (0, react_1.useRef)(null);
    const layersRef = (0, react_1.useRef)(enabledLayers);
    const prevLayersRef = (0, react_1.useRef)(enabledLayers);
    const wmLayersRef = (0, react_1.useRef)(worldMonitorLayers);
    const radarFilterRef = (0, react_1.useRef)(radarFilter);
    const reconLocationsRef = (0, react_1.useRef)(reconLocations);
    const fData = (0, react_1.useRef)(new Map());
    const fDataArr = (0, react_1.useRef)([]);
    const fSeen = (0, react_1.useRef)(new Map());
    const qData = (0, react_1.useRef)([]);
    const gdacsData = (0, react_1.useRef)([]);
    const airqData = (0, react_1.useRef)([]);
    const sRecs = (0, react_1.useRef)([]);
    const rData = (0, react_1.useRef)(new Map());
    const cData = (0, react_1.useRef)(new Map());
    const airportData = (0, react_1.useRef)([]);
    const airportsLoaded = (0, react_1.useRef)(false);
    const flightAnimationTick = (0, react_1.useRef)(0);
    const satsTickRef = (0, react_1.useRef)(0);
    const gilCities = (0, react_1.useRef)([]);
    const gilLoaded = (0, react_1.useRef)(false);
    const gilSelectedCity = (0, react_1.useRef)(null);
    const cameraCityKey = (0, react_1.useRef)("");
    const intelCityKey = (0, react_1.useRef)("");
    const cityIntelData = (0, react_1.useRef)([]);
    // UI state for bottom dashboard
    const intelMarkers = (0, react_1.useRef)([]);
    const countriesGeoRef = (0, react_1.useRef)(null);
    const trackedId = (0, react_1.useRef)(null);
    const flightHudRef = (0, react_1.useRef)(null);
    const flightHudTextRef = (0, react_1.useRef)(null);
    const panoData = (0, react_1.useRef)([]);
    const panoFetchKey = (0, react_1.useRef)("");
    const panoTimer = (0, react_1.useRef)(null);
    const pathPoints = (0, react_1.useRef)([]);
    const pathTimer = (0, react_1.useRef)(null);
    const wmInfraData = (0, react_1.useRef)(null);
    const wmMilitaryData = (0, react_1.useRef)(null);
    const wmGeoIntelData = (0, react_1.useRef)(null);
    const wmSanctionsData = (0, react_1.useRef)(null);
    const wmFeedData = (0, react_1.useRef)(null);
    const sigintData = (0, react_1.useRef)(null);
    const sigintFiltersRef = (0, react_1.useRef)(sigintFilters || {
        tab: "overview", severity: "low", confidence: 0, evidence: "all", cells: true, points: true, labels: true,
    });
    const weatherWindVectors = (0, react_1.useRef)([]);
    const weatherGrid = (0, react_1.useRef)(null);
    const weatherGridFetchedAt = (0, react_1.useRef)(0);
    const weatherFxCanvas = (0, react_1.useRef)(null);
    const weatherBaseCanvas = (0, react_1.useRef)(null);
    const weather_segment_canvases = (0, react_1.useRef)({ west: null, east: null });
    const weatherFxTimer = (0, react_1.useRef)(null);
    const weatherWindParticles = (0, react_1.useRef)([]);
    const weatherModeRef = (0, react_1.useRef)(weatherMode);
    const refreshRadarNowRef = (0, react_1.useRef)(null);
    const refreshUsgsNowRef = (0, react_1.useRef)(null);
    const refreshTleNowRef = (0, react_1.useRef)(null);
    const refreshRadioNowRef = (0, react_1.useRef)(null);
    const refreshCameraNowRef = (0, react_1.useRef)(null);
    const refreshWeatherNowRef = (0, react_1.useRef)(null);
    const refreshSigintNowRef = (0, react_1.useRef)(null);
    const refreshGdacsNowRef = (0, react_1.useRef)(null);
    const refreshAirqNowRef = (0, react_1.useRef)(null);
    const refreshGilNowRef = (0, react_1.useRef)(null);
    const refreshWmInfraNowRef = (0, react_1.useRef)(null);
    const refreshWmMilitaryNowRef = (0, react_1.useRef)(null);
    const refreshWmGeoIntelNowRef = (0, react_1.useRef)(null);
    const refreshWmSanctionsNowRef = (0, react_1.useRef)(null);
    const refreshWmFeedNowRef = (0, react_1.useRef)(null);
    const rebuildLayersRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        wmLayersRef.current = worldMonitorLayers;
        if (worldMonitorLayers && Object.values(worldMonitorLayers).some(Boolean)) {
            if (!wmFeedData.current && refreshWmFeedNowRef.current)
                refreshWmFeedNowRef.current();
        }
        rebuildLayersRef.current?.();
    }, [worldMonitorLayers]);
    const timersRef = (0, react_1.useRef)({ radar: null, usgs: null, satelliteCatalog: null, satelliteTick: null, radioRefresh: null, weatherRefresh: null, sigintRefresh: null, gdacsRefresh: null, airqRefresh: null, flightTick: null });
    (0, react_1.useEffect)(() => {
        const prev = prevLayersRef.current;
        layersRef.current = enabledLayers;
        if (!enabledLayers["gil"])
            onGilCount?.(0);
        if (!enabledLayers["gdacs"]) {
            gdacsData.current = [];
            onGdacsCount?.(0);
        }
        if (!enabledLayers["airq"]) {
            airqData.current = [];
            onAirqCount?.(0);
        }
        ...
        if (prev["radar"] === false && enabledLayers["radar"] !== false)
            refreshRadarNowRef.current?.();
        if (prev["usgs"] === false && enabledLayers["usgs"] !== false)
            refreshUsgsNowRef.current?.();
        if (prev["tle"] === false && enabledLayers["tle"] !== false)
            refreshTleNowRef.current?.();
        if (prev["radio"] === false && enabledLayers["radio"] !== false)
            refreshRadioNowRef.current?.();
        if (prev["camera"] !== true && enabledLayers["camera"] === true)
            refreshCameraNowRef.current?.();
        if (prev["noaa"] !== enabledLayers["noaa"])
            refreshWeatherNowRef.current?.();
        if (prev["sigint"] !== true && enabledLayers["sigint"] === true)
            refreshSigintNowRef.current?.();
        if (prev["gdacs"] !== true && enabledLayers["gdacs"] === true)
            refreshGdacsNowRef.current?.();
        if (prev["airq"] !== true && enabledLayers["airq"] === true)
            refreshAirqNowRef.current?.();
        if (prev["gil"] !== true && enabledLayers["gil"] === true)
            refreshGilNowRef.current?.();
    ...
    (0, react_1.useEffect)(() => {
        ...
        onFlightSelect?.(null);
        rebuildLayersRef.current?.();
        if (layersRef.current["radar"] !== false)
            refreshRadarNowRef.current?.();
    ...
    (0, react_1.useEffect)(() => {
        if (sigintFilters)
            sigintFiltersRef.current = sigintFilters;
        rebuildLayersRef.current?.();
    }, [sigintFilters]);
    (0, react_1.useEffect)(() => {
        const map = mapRef.current;
        if (!map || !activeImagery)
            return;
        const cfg = BASEMAP_STYLES[activeImagery] || BASEMAP_STYLES["hybrid"];
        if (map.getLayer("basemap-overlay-layer"))
            map.removeLayer("basemap-overlay-layer");
        if (map.getSource("basemap-overlay"))
            map.removeSource("basemap-overlay");
        ...
        if (activeImagery === "hybrid" || activeImagery === "aerial_labels") {
            ...
            if (map.getLayer("hybrid-labels-layer"))
                map.removeLayer("hybrid-labels-layer");
            if (map.getSource("hybrid-labels"))
                map.removeSource("hybrid-labels");
        }
    ...
    const captureSnapshot = (0, react_1.useCallback)(() => {
        const map = mapRef.current;
        if (!map)
            return;
        map.triggerRepaint();
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (!map)
            return;
        if (uiCommand.type === "zoomIn") {
            map.zoomIn({ duration: 300 });
            return;
        }
        if (uiCommand.type === "zoomOut") {
            map.zoomOut({ duration: 300 });
            return;
        }
        if (uiCommand.type === "resetView") {
            map.flyTo({ center: [0, 20], zoom: 2, duration: 900 });
            return;
        }
    ...
    (0, react_1.useEffect)(() => {
        ...
        const map = new maplibre_gl_1.default.Map({
            ...
            style: {
                ...
                layers: [
                    { id: "basemap-layer", type: "raster", source: "basemap", paint: {} },
                    ...(initialBasemap.overlayOn ? [
                        { id: "basemap-overlay-layer", type: "raster", source: "basemap-overlay", paint: {} }
                    ] : [])
                ],
        ...
        const handleDeckClick = (info, coordinate) => {
            if (!info || !info.object) {
                trackedId.current = null;
                pathPoints.current = [];
                if (pathTimer.current !== null) {
                    window.clearInterval(pathTimer.current);
            ...
            if (lid.startsWith("sigint-cells")) {
                ...
                if (cell)
                    onSigintSelect?.({ type: "cell", data: cell });
                onFlightSelect?.(null);
                onAirportSelect?.(null);
                onSatelliteSelect?.(null);
                onQuakeSelect?.(null);
                ...
                onSigintSelect?.({ type: "finding", data: obj });
                onFlightSelect?.(null);
                onAirportSelect?.(null);
                onSatelliteSelect?.(null);
                onQuakeSelect?.(null);
                ...
                const cityObj = lid.startsWith("wm") ? obj.city : obj;
                if (!cityObj)
                    return;
        ...
        const deck = new core_1.Deck({
            canvas: deckCanvas,
            // @ts-ignore - glOptions is valid but may be missing from types
            glOptions: { preserveDrawingBuffer: true },
            width: "100%",
            height: "100%",
            views: [isGlobe ? new core_1._GlobeView() : new core_1.MapView({ repeat: true })],
            ...
            if (tracked && tracked.startsWith("flight-")) {
                ...
                if (fl) {
                    ...
                    if (hud) {
                        ...
                        const parts = [cs];
                        if (fl100 != null)
                            parts.push("FL" + String(fl100).padStart(3, "0"));
                        parts.push(kts + " kts");
                        if (hdg != null)
                            parts.push("HDG " + String(hdg).padStart(3, "0") + "\u00B0");
                        if (flightHudTextRef.current)
                            flightHudTextRef.current.textContent = parts.join("  \u00B7  ");
        ...
        const reportView = () => {
            ...
            setStreetViewPos((prev) => (Math.abs(prev.lat - center.lat) < 0.0001 && Math.abs(prev.lon - center.lng) < 0.0001 && prev.zoom === svZoom
                ? prev
        ...
        const refreshPanoramas = async () => {
            if (layersRef.current["panorama"] === false) {
                if (panoData.current.length) {
                    panoData.current = [];
                    panoFetchKey.current = "";
                    rebuildLayersRef.current?.();
                }
                return;
            }
        ...
        const schedulePanoRefresh = () => {
            if (panoTimer.current)
                window.clearTimeout(panoTimer.current);
        ...
        const buildLayers = () => {
            ...
            if (ll["noaa"] === true && weather_grid && zoom >= 1.5 && weather_mode !== "radar") {
                ...
                layers.push(new layers_1.TextLayer({
                    ...
                    getText: d => (0, map_core_1.weather_value_label)(weather_mode, d),
            ...
            if (wmFeed && Object.values(wm).some(Boolean)) {
                ...
                if (countriesGeoRef.current && Object.keys(activeScores).length > 0) {
                    ...
                    layers.push(new layers_1.GeoJsonLayer({
                        ...
                        getFillColor: (f) => {
                            ...
                            if (!rec || !Object.keys(rec.layers || {}).some(k => activeWm.has(k)))
                                return [0, 0, 0, 0];
                ...
                for (const lid of ["cables", "pipelines", "trade_routes"]) {
                    if (!activeWm.has(lid))
                        continue;
                    const rows = pathRows.filter((x) => x.layer_id === lid);
                    if (!rows.length)
                        continue;
                    layers.push(new layers_1.PathLayer({
                        id: `wm-${lid}-unified`,
                        data: rows,
                        getPath: (d) => d.points,
                        getColor: (d) => wm_color(d.layer_id, lid === "cables" ? 190 : 220),
                ...
                for (const lid of wm_point_layers) {
                    if (!activeWm.has(lid))
                        continue;
                    const rows = pointRows.filter((x) => x.layer_id === lid);
                    if (!rows.length)
                        continue;
                    layers.push(new layers_1.ScatterplotLayer({
                        id: `wm-${lid}-unified`,
                        data: rows,
                        getPosition: (d) => [Number(d.lng ?? d.lon), Number(d.lat)],
                        getRadius: (d) => wm_radius(d.layer_id, d.severity),
                        getFillColor: (d) => wm_color(d.layer_id, d.severity === "critical" ? 245 : 215),
            ...
            if (!wmFeedData.current && countriesGeoRef.current && (wm.conflicts || wm.wars) && wmGeoIntelData.current) {
                ...
                for (const ev of events) {
                    if (ev.country_iso3)
                        activeIso3.add(ev.country_iso3.toUpperCase());
                        ...
                        if (nearest && nearest.iso)
                            activeIso3.add(nearest.iso.toUpperCase());
                    }
                }
            ...
            if (ll["sigint"] === true && sigintData.current) {
                ...
                const findings = sigintData.current.findings.filter((d) => {
                    if (sigint_severity_rank[d.severity] < sigint_severity_rank[cfg.severity] || d.confidence < cfg.confidence)
                        return false;
                    if (cfg.evidence !== "all" && d.evidence_class !== cfg.evidence)
                        return false;
                    if (cfg.tab === "navigation" && !navKinds.has(d.kind))
                        return false;
                    if (cfg.tab === "transponder" && !transKinds.has(d.kind))
                        return false;
                    return true;
                });
                const cells = sigintData.current.cells.filter((d) => sigint_severity_rank[d.severity] >= sigint_severity_rank[cfg.severity] && d.confidence >= cfg.confidence);
            ...
            if (ll["radar"] !== false) {
                ...
                layers.push(new layers_1.IconLayer({
                    id: "flights-icons",
                    ...
                    getIcon: (d) => ({ url: d._drIcon, width: 96, height: 96, anchorX: 48, anchorY: 48 }),
                    getSize: (d) => d.category === "MILITARY" ? 28 : 24,
                    sizeUnits: "pixels",
                ...
                if (airportsLoaded.current && zoom > 4) {
                    ...
                    layers.push(new layers_1.IconLayer({
                        ...
                        getIcon: (d) => ({ url: d.size === "large" ? AIRPORT_ICON_LARGE : AIRPORT_ICON_SMALL, width: 96, height: 96, anchorX: 48, anchorY: 96 }),
                        getSize: (d) => d.size === "large" ? 48 : d.size === "medium" ? 36 : 26,
                        sizeMinPixels: 10,
                        sizeMaxPixels: 60,
                        pickable: true,
                        loadOptions: { image: { type: "imagebitmap" } },
                    ...
                    if (zoom > 5) {
                        layers.push(new layers_1.TextLayer({
                            ...
                            getPosition: (d) => [Number(d.lon), Number(d.lat)],
                            getText: (d) => d.iata,
                            getSize: (d) => d.size === "large" ? 13 : d.size === "medium" ? 11 : 9,
                            getColor: (d) => d.size === "large" ? [251, 191, 36, 240] : d.size === "medium" ? [56, 189, 248, 240] : [148, 163, 184, 220],
                            getPixelOffset: [0, -40],
                            fontFamily: "'Space Mono', monospace",
                            fontWeight: "bold",
                            fontSettings: { sdf: true },
            ...
            if (ll["usgs"] !== false) {
                layers.push(new layers_1.ScatterplotLayer({
                    ...
                    getRadius: (d) => (d.mag !== null && d.mag >= 6 ? 80000 : d.mag !== null && d.mag >= 4 ? 50000 : 30000),
                    getFillColor: (d) => d.mag !== null && d.mag >= 6 ? [239, 68, 68, 200] : d.mag !== null && d.mag >= 4 ? [249, 115, 22, 200] : [251, 191, 36, 180],
                ...
                if (zoom > 3) {
                    layers.push(new layers_1.IconLayer({
                        ...
                        getSize: (d) => (d.mag !== null && d.mag >= 6 ? 32 : d.mag !== null && d.mag >= 4 ? 24 : 18),
                        sizeMinPixels: 8,
                        sizeMaxPixels: 40,
                        pickable: true,
                        loadOptions: { image: { type: "imagebitmap" } },
            ...
            if (ll["tle"] !== false) {
                layers.push(new layers_1.IconLayer({
                    ...
                    data: sRecs.current,
                    getPosition: (d) => [d.lon, d.lat],
                    getIcon: (d) => ({ url: d.category === "space-station" ? SPACE_STATION_ICON : d.category === "military" ? MILITARY_SATELLITE_ICON : SATELLITE_ICON, width: 96, height: 96, anchorX: 48, anchorY: 48 }),
                    getSize: (d) => d.category === "space-station" ? 38 : 28,
                    sizeMinPixels: 10,
                    sizeMaxPixels: 50,
                    pickable: true,
                    loadOptions: { image: { type: "imagebitmap" } },
                ...
                if (zoom > 2) {
                    layers.push(new layers_1.TextLayer({
                        ...
                        data: sRecs.current,
                        getPosition: (d) => [d.lon, d.lat],
                        getText: (d) => d.name || ("SAT-" + d.id),
                        getSize: 9,
                        getColor: (d) => d.category === "military" ? [248, 113, 113, 200] : [125, 211, 252, 200],
                        getPixelOffset: [0, -16],
                        fontFamily: "'Space Mono', monospace",
                        fontWeight: "600",
                        fontSettings: { sdf: true },
            ...
            if (ll["radio"] !== false) {
                ...
                if (radioArr.length > icon_count) {
                    layers.push(new layers_1.ScatterplotLayer({
                        ...
                        getPosition: (d) => [d.lon, d.lat],
                        getRadius: 15000,
                        getFillColor: (d) => d.source === "radio.garden" ? [56, 189, 248, 220] : [52, 211, 153, 210],
            ...
            if (ll["camera"] !== false && zoom >= 5) {
                ...
                if (zoom > 10 && cameraArr.length <= 12) {
                    layers.push(new layers_1.TextLayer({
                        ...
                        data: cameraArr,
                        getPosition: (d) => [d.lon, d.lat],
                        getText: (d) => d.name.toUpperCase().slice(0, 56),
                        getSize: 8,
            ...
            if (ll["gil"] === true) {
                ...
                layers.push(new layers_1.ScatterplotLayer({
                    ...
                    data: dots,
                    getPosition: (d) => [d.lon, d.lat],
                    getRadius: (d) => d.tier >= 3 ? 8000 : d.tier >= 2 ? 5000 : 3000,
                    getFillColor: (d) => d.tier >= 3 ? [165, 243, 252, 255] : d.tier >= 2 ? [103, 232, 249, 255] : [34, 211, 238, 255],
                    getLineColor: (d) => d.tier >= 4 ? [17, 24, 39, 240] : d.tier === 3 ? [69, 10, 10, 240] : [2, 6, 23, 240],
                    lineWidthMinPixels: 1,
                    stroked: true,
                    pickable: true,
                    radiusMinPixels: 2,
                ...
                layers.push(new layers_1.ScatterplotLayer({
                    ...
                    data: metros,
                    getPosition: (d) => [d.lon, d.lat],
                    getRadius: (d) => d.tier === 5 ? 15000 : 10000,
                    getFillColor: (d) => d.tier >= 5 ? [207, 250, 254, 255] : [165, 243, 252, 255],
                ...
                if (activePoints.length > 0) {
                    ...
                    layers.push(new layers_1.ScatterplotLayer({
                        ...
                        data: activePoints,
                        getPosition: (d) => [d.city.lon, d.city.lat],
                        getRadius: (d) => d.maxSeverity === "critical" ? 10000 : d.maxSeverity === "high" ? 7000 : 5000,
                ...
                if (!wmFeedData.current && wmInfraData.current) {
                    ...
                    if (wmLayersRef.current?.pipelines && infra.pipelines) {
                        layers.push(new layers_1.PathLayer({
                            ...
                            getPath: (d) => d.points,
                            getColor: (d) => d.type === "gas" ? [245, 158, 11, 220] : [217, 119, 6, 220],
                            getWidth: 2000,
                            widthMinPixels: 2,
                            widthMaxPixels: 6,
                            pickable: true,
                ...
                if (!wmFeedData.current && wmMilitaryData.current) {
                    ...
                    if (wmLayersRef.current?.conflicts && mil.conflicts) {
                        layers.push(new layers_1.ScatterplotLayer({
                            id: "wm-conflicts",
                            data: mil.conflicts,
                            getPosition: (d) => d.center ? d.center : [0, 0],
                            getRadius: 25000,
                ...
                if (zoom > 2) {
                    const labeled = visible.filter(c => {
                        if (zoom > 8)
                            return c.tier >= 3 || (c.capital && c.pop >= 80000);
                        if (zoom > 5)
                            return c.tier >= 3;
                        if (zoom > 3)
                            return c.tier >= 4 || (c.capital && c.pop >= 500000);
                        return c.tier === 5 || (c.capital && c.pop >= 3000000);
                    }).slice(0, 180);
                    layers.push(new layers_1.TextLayer({
                        id: "gil-labels",
                        data: labeled,
                        getPosition: (d) => [d.lon, d.lat],
                        getText: (d) => `${d.name}${d.capital ? " CAP" : ""}`,
                        getSize: (d) => d.tier >= 5 ? 12 : d.tier >= 4 ? 11 : 9,
                        getColor: (d) => d.tier >= 4 ? [254, 243, 199, 255] : [254, 202, 202, 255],
                        getPixelOffset: [0, -12],
                        fontFamily: "'Space Mono', monospace",
                        fontWeight: "700",
                        fontSettings: { sdf: true },
            ...
            if (onWmCounts) {
                const newCounts = { conflicts: 0, wars: 0, civil_unrest: 0, violence: 0, humanitarian: 0, nuclear: 0, bases: 0, pipelines: 0, cables: 0, sanctions: 0 };
                if (wmFeedData.current?.counts) {
                    ...
                    for (const ev of wmGeoIntelData.current.events) {
                        if (ev.layer_id === "conflicts" || ev.layer_id === "wars") {
                            newCounts.conflicts++;
                            if (ev.severity === "critical")
                                newCounts.wars++;
                        }
                        else if (ev.layer_id === "civil_unrest")
                            newCounts.civil_unrest++;
                        else if (ev.layer_id === "violence")
                            newCounts.violence++;
                        else if (ev.layer_id === "humanitarian")
                            newCounts.humanitarian++;
                    }
                }
                if (!wmFeedData.current && wmInfraData.current) {
                ...
                if (deckRef._lastWmCounts !== currentCountsStr) {
                    deckRef._lastWmCounts = currentCountsStr;
        ...
        const refreshFlights = async () => {
            ...
            try {
                ...
                const data = (await res.json());
                if (activeFilter !== radarFilterRef.current)
                    return;
                ...
                for (const [eid, ls] of fSeen.current.entries()) {
                    if (now - ls > FLIGHT_STALE_AFTER_MS) {
                        fSeen.current.delete(eid);
                        fData.current.delete(eid);
                    }
                }
                ...
                console.warn("[WORLDVIEW] Radar:", err);
            }
        };
        const refreshQuakes = async () => {
            if (layersRef.current["usgs"] === false) {
                onUsgsCount?.(0);
                return;
            }
            try {
                ...
                console.warn("[WORLDVIEW] USGS:", err);
            }
        };
        const refreshSigint = async () => {
            ...
            try {
                ...
                console.warn("[WORLDVIEW] SIGINT:", err);
            }
        };
        const refreshGdacs = async () => {
            if (layersRef.current["gdacs"] !== true) {
                onGdacsCount?.(0);
                return;
            }
            try {
                ...
                console.warn("[WORLDVIEW] GDACS:", err);
            }
        };
        const refreshAirq = async () => {
            if (layersRef.current["airq"] !== true) {
                onAirqCount?.(0);
                return;
            }
            try {
                ...
                console.warn("[WORLDVIEW] Air quality:", err);
            }
        };
        const rebuildSats = async () => {
            if (layersRef.current["tle"] === false) {
                sRecs.current = [];
                return;
            }
            try {
                ...
                for (const tle of sats) {
                    try {
                        ...
                        if (sr.error !== satellite.SatRecError.None)
                            continue;
                ...
                console.warn("[WORLDVIEW] Satellites:", err);
            }
        };
        const tickSats = () => {
            const now = new Date();
            ...
            for (const r of sRecs.current) {
                ...
                if (p) {
                    newRecs.push({ ...r, lat: p.lat, lon: p.lon });
                }
                else {
                    newRecs.push(r);
                }
        ...
        const refreshWmInfra = async () => {
            ...
            try {
                ...
                console.warn("[WORLDVIEW] WM Infra:", err);
            }
        };
        const refreshWmMilitary = async () => {
            ...
            try {
                ...
                console.warn("[WORLDVIEW] WM Military:", err);
            }
        };
        const refreshWmGeoIntel = async () => {
            ...
            try {
                ...
                console.warn("[WORLDVIEW] Geo Intel:", err);
            }
        };
        const refreshWmSanctions = async () => {
            ...
            try {
                ...
                console.warn("[WORLDVIEW] WM Sanctions:", err);
            }
        };
        const refreshWmFeed = async () => {
            ...
            try {
                ...
                console.warn("[WORLDVIEW] WM Unified:", err);
            }
        };
        let radio_refresh_generation = 0;
        const refreshRadio = async () => {
            if (layersRef.current["radio"] === false) {
                onRadioCount?.(0);
                return;
            }
            ...
            const ingest = (stations, max) => {
                if (generation !== radio_refresh_generation)
                    return;
                let source_added = 0;
                for (const st of stations) {
                    ...
                    if (!sid || seen.has(sid) || !nm || !Number.isFinite(lat) || !Number.isFinite(lon))
                        continue;
            ...
            const load_source = async (url, max) => {
                try {
                    const res = await fetch(url, { cache: "no-store" });
                    if (!res.ok)
                        throw new Error(String(res.status));
        ...
        const refreshCityIntel = async (city) => {
            ...
            if (intelCityKey.current === nextKey)
                return;
            ...
            try {
                ...
                if (!res.ok)
                    throw new Error("intel fetch failed");
                const data = await res.json();
                intelMarkers.current.forEach(m => m.remove());
                intelMarkers.current = [];
                ...
                console.warn("[WORLDVIEW] Intel:", err);
            }
        };
        const refreshCamerasForCity = async (city) => { return; };
        const loadAirports = async () => {
            ...
            try {
                ...
                if (!res.ok)
                    throw new Error("airports.json " + res.status);
                const data = (await res.json());
                ...
                console.error("[WORLDVIEW] Airports:", err);
            }
        };
        const loadCities = async () => {
            if (gilLoaded.current) {
                buildLayers();
                return;
            }
            try {
                ...
                if (!res.ok)
                    throw new Error("cities.json " + res.status);
                ...
                for (let i = 0; i < cities.length; i++) {
                    ...
                    if (!raw.name || !Number.isFinite(lat) || !Number.isFinite(lon))
                        continue;
                    ...
                    records.push({
                        ...
                        id: "gil-" + i, tier, score: tier * 100000000 + pop + (raw.capital ? 25000000 : 0),
                    });
                }
                ...
                console.error("[WORLDVIEW] GIL cities:", err);
            }
        };
        const loadCountriesGeo = async () => {
            ...
            try {
                ...
                if (res.ok)
                    countriesGeoRef.current = await res.json();
            }
            catch (err) {
                console.error("[WORLDVIEW] countries.geojson:", err);
            }
        };
        const remove_weather_layer = (layer, source) => {
        ...
        const clear_weather = () => {
            ...
            for (const segment of map_core_1.weather_canvas_segments) {
        ...
        const ensure_weather_canvas = () => {
            ...
            if (!map.getLayer("weather-map-dim-layer")) {
                map.addLayer({
                    ...
                    paint: {
                        "fill-color": "#020812",
                        "fill-opacity": map_core_1.weather_visuals.map_dim_opacity,
                    },
                });
            }
            ...
            if (span < 300) {
                ...
                const source = map.getSource("weather-field-local");
                if (source)
                    source.setCoordinates(coordinates);
                else
                    map.addSource("weather-field-local", {
                        type: "canvas",
                        canvas,
                        animate: true,
                        coordinates,
                ...
                if (!map.getLayer("weather-field-local-layer")) {
                    map.addLayer({
                        ...
                        paint: { "raster-opacity": map_core_1.weather_visuals.field_opacity, "raster-fade-duration": 0 },
                    });
                }
            }
            else {
        ...
        const sync_weather_layers = () => {
            ...
            if (map.getLayer("weather-map-dim-layer")) {
                map.setPaintProperty("weather-map-dim-layer", "fill-opacity", visible ? map_core_1.weather_visuals.map_dim_opacity : 0);
            }
            if (map.getLayer("weather-field-local-layer")) {
                map.setPaintProperty("weather-field-local-layer", "raster-opacity", visible ? (mode === "precipitation" ? map_core_1.weather_visuals.precipitation_opacity : map_core_1.weather_visuals.field_opacity) : 0);
            }
            for (const segment of map_core_1.weather_canvas_segments) {
                const layer_id = `weather-field-${segment.id}-layer`;
                if (map.getLayer(layer_id)) {
                    map.setPaintProperty(layer_id, "raster-opacity", visible ? (mode === "precipitation" ? map_core_1.weather_visuals.precipitation_opacity : map_core_1.weather_visuals.field_opacity) : 0);
                }
            }
            if (map.getLayer("rain-layer")) {
                const opacity = mode === "radar" ? map_core_1.weather_visuals.radar_opacity : mode === "precipitation" ? 0.2 : 0;
                map.setPaintProperty("rain-layer", "raster-opacity", opacity);
            }
        };
        const grid_vector = (row, col) => {
            const grid = weatherGrid.current;
            if (!grid)
                return null;
            return weatherWindVectors.current[row * grid.cols + col] || null;
        };
        const sample_weather = (lat, lon) => {
            const grid = weatherGrid.current;
            if (!grid)
                return null;
        ...
        const render_weather_base = () => {
            ...
            for (let y = 0; y < h; y += step) {
                ...
                for (let x = 0; x < w; x += step) {
                    ...
                    if (!sample)
                        continue;
                    const value = (0, map_core_1.weather_value)(mode, sample);
                    const [r, g, b] = (0, map_core_1.weather_color)(mode, value, 255);
        ...
        const draw_weather_frame = () => {
            ...
            ctx.clearRect(0, 0, w, h);
            ctx.filter = `blur(${map_core_1.weather_field_blur_px}px)`;
            ctx.drawImage(base, -map_core_1.weather_field_blur_px, -map_core_1.weather_field_blur_px, w + map_core_1.weather_field_blur_px * 2, h + map_core_1.weather_field_blur_px * 2);
            ctx.filter = "none";
            const mode = weatherModeRef.current;
            ...
            while (particles.length < map_core_1.weather_visuals.particle_count) {
            ...
            for (const p of particles) {
                ...
                const dx = (v.u_ms || 0) * map_core_1.weather_visuals.particle_flow_scale * amp;
                const dy = -(v.v_ms || 0) * map_core_1.weather_visuals.particle_flow_scale * amp;
                const x_step = (0, map_core_1.weather_particle_x)(p.x, dx, w);
                const y_step = (0, map_core_1.weather_particle_y)(p.y, dy, h, Math.random() * h);
                p.x = x_step.x;
                p.y = y_step.y;
                p.age++;
                ...
                const alpha = map_core_1.weather_visuals.particle_alpha_min + mag * (map_core_1.weather_visuals.particle_alpha_max - map_core_1.weather_visuals.particle_alpha_min);
                ctx.strokeStyle = `rgba(242,251,255,${alpha})`;
                ctx.lineWidth = map_core_1.weather_visuals.particle_width_min + mag * (map_core_1.weather_visuals.particle_width_max - map_core_1.weather_visuals.particle_width_min);
                ctx.beginPath();
                ctx.moveTo(ox - dx * map_core_1.weather_visuals.particle_trail, oy - dy * map_core_1.weather_visuals.particle_trail);
                ctx.lineTo(p.x, p.y);
                ctx.stroke();
            }
            ctx.shadowBlur = 0;
        ...
        const refreshWeather = async () => {
            ...
            if (needs_field) {
                ...
                const fresh = Date.now() - weatherGridFetchedAt.current < 8 * 60000;
                if (!covered || !detailed || !fresh) {
                    ...
                    try {
                        ...
                        if (res.ok) {
                            const data = await res.json();
                            if (seq !== weather_request_seq || !data.grid)
                                return;
                        ...
                        console.warn("[WORLDVIEW] weather grid:", err);
                    }
                    if (seq !== weather_request_seq)
                        return;
            ...
            if (mode === "wind" || mode === "wind_gusts") {
                if (weatherFxTimer.current === null)
                    weatherFxTimer.current = window.setInterval(draw_weather_frame, 45);
            }
            ...
            if (mode === "radar" || mode === "precipitation") {
                try {
                    ...
                    console.warn("[WORLDVIEW] weather radar:", err);
                }
            }
            sync_weather_layers();
            rebuildLayersRef.current?.();
        };
        let weather_view_timer = null;
        const refresh_weather_view = () => {
            const mode = weatherModeRef.current;
            if (layersRef.current["noaa"] !== true || mode === "radar")
                return;
            if (weather_view_timer !== null)
                window.clearTimeout(weather_view_timer);
        ...
        map.on("load", () => {
            ...
            if (enabledLayers["gil"] === true)
                void loadCities();
            if (wmLayersRef.current && Object.values(wmLayersRef.current).some(Boolean))
                void refreshWmFeed();
        });
        ...
        const recordPath = () => {
            ...
            if (pathPoints.current.length > PATH_MAX_POINTS)
        ...
        return () => {
            ...
            if (pathTimer.current !== null)
                window.clearInterval(pathTimer.current);
            if (tmrs.radar !== null)
                window.clearInterval(tmrs.radar);
            if (tmrs.usgs !== null)
                window.clearInterval(tmrs.usgs);
            if (tmrs.satelliteCatalog !== null)
                window.clearInterval(tmrs.satelliteCatalog);
            if (tmrs.satelliteTick !== null)
                window.clearInterval(tmrs.satelliteTick);
            if (tmrs.radioRefresh !== null)
                window.clearInterval(tmrs.radioRefresh);
            if (tmrs.weatherRefresh !== null)
                window.clearInterval(tmrs.weatherRefresh);
            if (tmrs.sigintRefresh !== null)
                window.clearInterval(tmrs.sigintRefresh);
            if (tmrs.gdacsRefresh !== null)
                window.clearInterval(tmrs.gdacsRefresh);
            if (tmrs.airqRefresh !== null)
                window.clearInterval(tmrs.airqRefresh);
            if (weatherFxTimer.current !== null)
                window.clearInterval(weatherFxTimer.current);
            if (weather_view_timer !== null)
                window.clearTimeout(weather_view_timer);
    ...
    (0, react_1.useEffect)(() => {
        ...
        const updateProj = () => {
            if (isGlobe) {
                ...
                deckRef.current?.setProps({ views: [new core_1._GlobeView()] });
            }
            else {
                map.setProjection({ type: "mercator" });
                deckRef.current?.setProps({ views: [new core_1.MapView({ repeat: true })] });
            }
        };
        if (map.isStyleLoaded())
            updateProj();
        else
            map.once("styledata", updateProj);
    }, [isGlobe]);
    (0, react_1.useEffect)(() => {
        if (!mapRef.current)
            return;
        ...
        if (map.isStyleLoaded())
            ...
            map.once("styledata", updateKarta);
    }, [streetViewMode]);
    ...
    return (<div className="absolute inset-0 w-full h-full bg-black" style={{ filter: selectedPano ? "none" : styleFilter }}>
            <div ref={containerRef} style={{ width: "100%", height: "100%", position: "relative" }}/>
            <div className="absolute inset-0 pointer-events-none map-grid opacity-30"/>

            {snapshot && (<div className="absolute inset-0 z-[120] flex items-center justify-center bg-black/85 p-6 backdrop-blur-md" onClick={() => setSnapshot(null)}>
                    <div className="relative flex max-h-full w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-cyan-400/25 bg-stone-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.75)]" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between border-b border-white/10 px-5 py-3">
                            ...
                            <div className="flex items-center gap-2">
                                <a href={snapshot.url} download={`akashic_snapshot_${Date.now()}.png`} className="rounded-full border border-cyan-400/40 bg-cyan-500/15 px-4 py-1.5 font-mono text-[0.6rem] font-bold uppercase tracking-[0.18em] text-cyan-200 transition-colors hover:bg-cyan-500/25">
                                    Download PNG
                                </a>
                                <button onClick={() => setSnapshot(null)} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 font-mono text-[0.6rem] font-bold uppercase tracking-[0.18em] text-stone-300 transition-colors hover:bg-white/10">
                                    Close
                                </button>
                            </div>
                        </div>
            ...
            {selectedPano && (<div className="absolute inset-0 z-[130] flex flex-col bg-black">
                    <div className="flex items-center justify-between border-b border-violet-400/25 bg-stone-950/95 px-5 py-3">
                        ...
                        <div className="flex items-center gap-2">
                            <a href={selectedPano.pageUrl} target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/10 bg-white/5 px-4 py-1.5 font-mono text-[0.6rem] font-bold uppercase tracking-[0.18em] text-stone-300 transition-colors hover:bg-white/10">
                                Open ↗
                            </a>
                            <button onClick={() => setSelectedPano(null)} className="flex items-center gap-2 rounded-full border border-violet-400/40 bg-violet-500/15 px-4 py-1.5 font-mono text-[0.6rem] font-bold uppercase tracking-[0.18em] text-violet-200 transition-colors hover:bg-violet-500/25">
                    ...
                    <iframe key={selectedPano.handle} src={selectedPano.embedUrl} title={selectedPano.title} className="h-full w-full flex-1 border-0 bg-black" allow="accelerometer; gyroscope; fullscreen; xr-spatial-tracking" allowFullScreen referrerPolicy="no-referrer-when-downgrade"/>
                </div>)}
            ...
            {(activeStyle === "crt" || activeStyle === "nightvision") && (<div className="absolute inset-0 pointer-events-none" style={{
                zIndex: 4,
            ...
            {activeStyle !== "standard" && (<div className="absolute inset-0 pointer-events-none" style={{
                zIndex: 5,
            ...
            {activeStyle === "radar" && (<div className="absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden" style={{ zIndex: 5 }}>
            ...
            {streetViewMode && (<div className="absolute bottom-16 left-[28rem] w-96 h-96 rounded-lg shadow-2xl border border-slate-600 overflow-hidden z-50">
                    <StreetViewMap_1.StreetViewMap initialLat={streetViewPos.lat} initialLon={streetViewPos.lon}/>
                </div>)}
        </div>);
}
