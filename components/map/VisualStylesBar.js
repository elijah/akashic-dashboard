...
exports.default = VisualStylesBar;
const STYLES = [
...
function VisualStylesBar({ activeStyle, setActiveStyle }) {
    return (<div className="panel-flat flex items-center w-full shrink-0 h-14 bg-m3-surface border-t border-m3-outline-variant px-4 gap-2 rounded-none border-b-0 border-l-0 border-r-0">
            ...
            return (<button key={s.id} onClick={() => !s.locked && setActiveStyle(s.id)} className={`flex items-center justify-between gap-3 px-3 h-8 rounded-md border transition-all duration-200 outline-none ${isActive
        ...
        ].map((stat) => (<div key={stat.label} className="flex flex-col items-center">
