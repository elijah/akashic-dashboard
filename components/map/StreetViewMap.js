...
function StreetViewMap({ initialLat = 0, initialLon = 0, }) {
    const canvasRef = (0, react_1.useRef)(null);
    const [photo, setPhoto] = (0, react_1.useState)(null);
    const [photos, setPhotos] = (0, react_1.useState)([]);
    const [photoIdx, setPhotoIdx] = (0, react_1.useState)(0);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [error, setError] = (0, react_1.useState)(null);
    const imgRef = (0, react_1.useRef)(null);
    const dragRef = (0, react_1.useRef)({ dragging: false, startX: 0, offsetX: 0, currentOffsetX: 0 });
    const fetchNearby = (0, react_1.useCallback)(async (lat, lon) => {
        setLoading(true);
        setError(null);
        try {
            ...
            if (!res.ok)
                throw new Error(`KartaView ${res.status}`);
    ...
    const getCandidateUrls = (p) => {
        const urls = [];
        if (p.imageProcUrl)
            urls.push(p.imageProcUrl);
        if (p.imageThUrl)
            urls.push(p.imageThUrl);
        if (p.imageLthUrl)
            urls.push(p.imageLthUrl);
        if (p.fileurlProc)
            urls.push(p.fileurlProc);
        if (p.fileurlTh)
            urls.push(p.fileurlTh);
        if (p.fileurlLTh)
            urls.push(p.fileurlLTh);
        if (p.fileurl) {
    ...
    const drawImage = (ctx, img, cw, ch, offsetX, is360) => {
        ctx.fillStyle = "#111";
    ...
    const onPointerDown = (e) => {
        if (!photo || photo.projection !== "SPHERE")
            return;
    ...
    const onPointerMove = (e) => {
        if (!dragRef.current.dragging || !canvasRef.current || !imgRef.current)
            return;
        ...
        const ctx = canvas.getContext("2d");
        if (ctx)
            drawImage(ctx, imgRef.current, canvas.width, canvas.height, dragRef.current.currentOffsetX, true);
    };
    const onPointerUp = () => {
    ...
    return (<div style={{ width: "100%", height: "100%", position: "relative", backgroundColor: "#111", overflow: "hidden" }}>
            
            <canvas ref={canvasRef} style={{ width: "100%", height: "100%", cursor: photo?.projection === "SPHERE" ? "grab" : "default", display: photo ? "block" : "none" }} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp}/>

            
            {(loading || error) && (<div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "#aaa", fontSize: 13, textAlign: "center", padding: 16 }}>
                    {loading ? "Loading street view..." : error}
                </div>)}

            
            {photo && photos.length > 1 && (<div style={{ position: "absolute", bottom: 8, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 8, zIndex: 30 }}>
                    <button onClick={goPrev} disabled={photoIdx <= 0} style={{ background: "rgba(0,0,0,0.7)", color: "#fff", border: "1px solid #555", borderRadius: 4, padding: "4px 12px", cursor: photoIdx > 0 ? "pointer" : "not-allowed", fontSize: 13 }}>
                        ◀ Prev
                    </button>
                    <span style={{ color: "#ccc", fontSize: 12, alignSelf: "center" }}>
                        {photoIdx + 1}/{photos.length}
                    </span>
                    <button onClick={goNext} disabled={photoIdx >= photos.length - 1} style={{ background: "rgba(0,0,0,0.7)", color: "#fff", border: "1px solid #555", borderRadius: 4, padding: "4px 12px", cursor: photoIdx < photos.length - 1 ? "pointer" : "not-allowed", fontSize: 13 }}>
                        Next ▶
                    </button>
                </div>)}

            
            {photo && (<div style={{ position: "absolute", top: 0, left: 0, right: 0, background: "rgba(0,0,0,0.65)", color: "#ddd", fontSize: 11, padding: "4px 8px", zIndex: 30, display: "flex", justifyContent: "space-between" }}>
