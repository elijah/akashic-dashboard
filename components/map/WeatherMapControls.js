...
exports.WeatherMapControls = void 0;
const lucide_react_1 = require("lucide-react");
const map_core_1 = require("@/lib/weather/map-core");
const icons = {
    radar: lucide_react_1.Radar,
    precipitation: lucide_react_1.CloudRain,
    wind: lucide_react_1.Wind,
    wind_gusts: lucide_react_1.Zap,
    temperature: lucide_react_1.Thermometer,
    humidity: lucide_react_1.Droplets,
    pressure: lucide_react_1.Gauge,
};
const WeatherMapControls = ({ mode, on_mode, on_close }) => {
    const meta = map_core_1.weather_mode_meta[mode];
    return <>
    <section className="weather-map-controls pointer-events-auto fixed top-20 z-40 w-56 overflow-hidden rounded-lg border border-sky-300/20 bg-[#102642]/90 text-slate-100 shadow-2xl backdrop-blur-xl">
      ...
      <div className="p-1.5">
        {map_core_1.weather_map_modes.map(id => {
            const Icon = icons[id];
            const active = id === mode;
            return <button key={id} onClick={() => on_mode(id)} className={`flex h-9 w-full items-center gap-3 rounded-md px-2.5 text-left text-xs transition ${active ? "bg-sky-200/20 text-white shadow-[inset_2px_0_0_#7dd3fc]" : "text-slate-300 hover:bg-white/[0.07] hover:text-white"}`}>
            <Icon className={`size-4 ${active ? "text-sky-200" : "text-slate-400"}`}/>
            <span className="flex-1">{map_core_1.weather_mode_meta[id].label}</span>
    ...
    <section className="pointer-events-none fixed bottom-24 left-1/2 z-40 w-[min(34rem,calc(100vw-2rem))] -translate-x-1/2 rounded-lg border border-sky-200/20 bg-[#102642]/88 px-3 py-2 shadow-2xl backdrop-blur-xl">
      ...
      <div className="flex h-4 overflow-hidden rounded-sm border border-white/10">
        {meta.legend.map((item, idx) => <div key={`${mode}-${item.v}`} className="relative flex-1" style={{ background: item.color }}>
...
exports.WeatherMapControls = WeatherMapControls;
