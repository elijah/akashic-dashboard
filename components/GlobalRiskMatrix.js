...
exports.GlobalRiskMatrix = void 0;
const react_1 = require("react");
const GlobalRiskMatrix = ({ onClose }) => {
    const [scores, setScores] = (0, react_1.useState)([]);
    const [correlations, setCorrelations] = (0, react_1.useState)([]);
    const [stats, setStats] = (0, react_1.useState)({ assets: 0, entities: 0 });
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [view, setView] = (0, react_1.useState)("risk");
    ...
    (0, react_1.useEffect)(() => { fetch_data(); }, []);
    return (<div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
      ...
      <div className="relative w-[95vw] max-w-[1400px] h-[90vh] bg-[#050505] border border-orange-900/50 shadow-[0_0_80px_rgba(249,115,22,0.15)] flex flex-col pointer-events-auto font-mono text-stone-300 rounded-xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#110500] to-[#050505]">
          ...
          {loading ? (<div className="w-full h-full flex flex-col items-center justify-center gap-4">
               ...
               <div className="text-xs font-bold text-orange-500 uppercase tracking-widest animate-pulse">COMPUTING MULTI-DIMENSIONAL RISK...</div>
             </div>) : view === "risk" ? (<div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {scores.length === 0 ? (<div className="text-center text-stone-500 py-10 uppercase text-xs">NO ACTIVE RISK SCORES</div>) : scores.map(s => {
                let col = "text-yellow-500";
                let bcol = "border-yellow-900/30";
                let bgcol = "bg-yellow-900/10";
                if (s.severity === "critical") {
                    col = "text-orange-500";
                    bcol = "border-orange-500/50";
                    bgcol = "bg-orange-500/10";
                }
                else if (s.severity === "high") {
                    col = "text-red-500";
                    bcol = "border-red-900/50";
                    bgcol = "bg-red-900/10";
                }
                return (<div key={s.id} className={`flex items-stretch bg-[#0a0a0a] border ${bcol} rounded overflow-hidden`}>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {correlations.length === 0 ? (<div className="col-span-2 text-center text-stone-500 py-10 uppercase text-xs">NO SIGNIFICANT CORRELATIONS DETECTED IN LAST 48 HOURS</div>) : correlations.map((c, i) => (<div key={i} className="bg-[#0a0a0a] border border-fuchsia-900/40 rounded-xl p-4 flex flex-col gap-4 shadow-lg shadow-fuchsia-900/10">
...
exports.GlobalRiskMatrix = GlobalRiskMatrix;
