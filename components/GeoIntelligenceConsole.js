...
exports.GeoIntelligenceConsole = void 0;
...
const sec = (title, body, extra = "") => (<div className={`mb-3 p-2.5 border border-white/10 rounded bg-white/[0.02] ${extra}`}>
    <div className="text-[11px] tracking-widest text-stone-500 mb-2">{title}</div>
    {body}
  </div>);
const GeoIntelligenceConsole = ({ data, loading }) => {
    const [tab, set_tab] = (0, react_1.useState)("overview");
    if (loading || !data)
        return (<div className="w-full min-h-[400px] flex items-center justify-center font-mono text-stone-500 text-xs">
    ...
    return (<div className="w-full font-mono text-stone-300 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <div className="flex flex-col">
          ...
          {sec("balance vector", (<div className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="text-[10px] text-stone-500 mb-1">pressures</div>
                  {data.risk.pressures?.map(x => (<div key={x.label} className="grid grid-cols-[60px_24px_1fr] gap-2 items-center mb-1">
                ...
                <div>
                  ...
                  {data.risk.buffers?.map(x => (<div key={x.label} className="grid grid-cols-[60px_24px_1fr] gap-2 items-center mb-1">
                      <span className="text-[10px] text-stone-400">{x.label}</span>
                      <span className="text-[10px] tabular-nums">{x.value.toFixed(2)}</span>
                      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className={`h-full ${x.color}`} style={{ width: `${x.value * 100}%` }}/>
        ...
        <div className="flex flex-col">
          {sec("actors & markets", (<div className="flex flex-col">
              {[...data.markets, ...data.crypto].slice(0, 6).map(a => {
                ...
                return (<div key={a.id} className="grid grid-cols-[1fr_auto_auto] gap-2 items-center py-1.5 border-b border-dashed border-white/10 last:border-0">
                    <div>
        ...
        <div className="flex flex-col">
          {sec("narrative brief", (<div className="flex flex-col gap-2">
              ...
              {data.brief.drivers.map(d => (<div key={d} className="text-[10px] flex gap-2"><span className="text-stone-600">▸</span> {d.toLowerCase()}</div>))}
            </div>))}

          {sec("transmission health", (<div className="flex flex-col">
              {data.source_health.source_groups.map(g => (<div key={g.name} className="grid grid-cols-[1fr_auto_auto] gap-2 items-center py-1.5 border-b border-dashed border-white/10 last:border-0">
                  <div>
        ...
        <div className="flex flex-col">
          {sec("watchlist triggers", (<div className="flex flex-col">
              {data.events.slice(0, 6).map(e => (<div key={e.id} className="py-1.5 border-b border-dashed border-white/10 last:border-0 text-[10px]">
          ...
          {data.correlations.length > 0 && sec("correlations", (<div className="flex flex-col gap-1">
              {data.correlations.map(c => (<div key={c.id} className="text-[10px]">
                  <span className="text-stone-400">▸</span> {c.interpretation.toLowerCase()}
                </div>))}
            </div>), "border-purple-500/30 bg-purple-500/[0.02]")}
        </div>

      </div>
    </div>);
};
exports.GeoIntelligenceConsole = GeoIntelligenceConsole;
