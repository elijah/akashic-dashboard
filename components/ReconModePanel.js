...
exports.ReconModePanel = ReconModePanel;
const react_1 = require("react");
const init_stats = { checked: 0, found: 0, not_found: 0, error: 0, total: 0 };
...
const source_quality = (r) => {
    ...
    if (txt(info.name))
        score += 5;
    if (txt(info.username))
        score += 4;
    if (txt(info.bio))
        score += 4;
    if (txt(info.location))
        score += 3;
    if (txt(info.website || info.url))
        score += 2;
    if (info.followers !== undefined || info.following !== undefined)
        score += 3;
    score += Math.min(4, clean_detail_entries(info).length);
    const rank = pfprank(r);
    if (rank < 999)
        score += 5 - Math.min(4, rank);
    return score;
};
const profile_fields = (r) => {
    ...
    ].map(([key, val]) => [String(key), txt(val)]).filter(([, val]) => val);
};
function ReconResultsDetail({ query, found, fetches, locations, counts, hero }) {
    const cats = Array.from(found.reduce((m, r) => m.set(r.category, (m.get(r.category) || 0) + 1), new Map()))
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8);
    ...
    const source_cards = [...found]
        ...
        .slice(0, 24);
    return (<article className="mb-3 overflow-hidden rounded-2xl border border-cyan-300/15 bg-gradient-to-b from-cyan-950/20 via-black/35 to-black/25 shadow-[0_0_35px_rgba(34,211,238,0.08)]">
      <div className="border-b border-white/10 px-3 py-3">
        ...
        <div className="mt-3 grid grid-cols-6 gap-1.5">
          {[
            ...
            ["error", counts.error],
        ].map(([k, v]) => (<div key={k} className="rounded-lg border border-white/5 bg-black/35 px-2 py-2 text-center">
      ...
      <div className="grid gap-2 p-3">
        <div className="grid grid-cols-[5.5rem_1fr] gap-3">
          <div className="overflow-hidden rounded-xl border border-white/10 bg-stone-950">
            {hero?.avatar ? (<img src={hero.avatar} alt="" className="h-24 w-full object-cover" referrerPolicy="no-referrer" onError={imgerr}/>) : (<div className="flex h-24 items-center justify-center font-mono text-[0.58rem] uppercase tracking-widest text-stone-600">no pfp</div>)}
          </div>
          <div className="min-w-0 rounded-xl border border-white/5 bg-black/25 p-2.5">
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {(names.length ? names : [query]).map(x => (<span key={x} className="max-w-full truncate rounded-full border border-cyan-300/15 bg-cyan-300/10 px-2 py-1 font-mono text-[0.58rem] text-cyan-100">{x}</span>))}
            </div>
            {!!avatars.length && (<div className="mt-2 flex -space-x-2">
                {avatars.map(x => (<img key={`${x.source}-${x.img}`} src={x.img} alt="" title={`${x.source} / ${host(x.url)}`} className="h-7 w-7 rounded-full border border-stone-950 bg-stone-900 object-cover" referrerPolicy="no-referrer" onError={imgerr}/>))}
              </div>)}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <section className="rounded-xl border border-white/5 bg-black/25 p-2.5">
            ...
            <div className="space-y-1.5">
              {(cats.length ? cats : [["no hits", 0]]).map(([k, v]) => (<div key={k} className="grid grid-cols-[1fr_2rem] items-center gap-2">
                  <div className="min-w-0">
          ...
          <section className="rounded-xl border border-white/5 bg-black/25 p-2.5">
            ...
            <div className="space-y-1.5">
              {locs.length ? locs.map(x => (<div key={x.id} className="rounded-lg border border-emerald-400/10 bg-emerald-400/5 px-2 py-1.5">
                ...
                </div>)) : <p className="rounded-lg border border-white/5 bg-stone-950/50 px-2 py-4 text-center text-xs text-stone-600">no geocoded public location returned</p>}
            </div>
          </section>
        </div>

        {!!source_cards.length && (<section className="rounded-xl border border-white/5 bg-black/25 p-2.5">
            <div className="mb-2 flex items-center justify-between">
            ...
            <div className="space-y-2">
              {source_cards.map(r => {
                ...
                return (<article key={`card-${r.source}-${r.url}`} className="rounded-xl border border-white/10 bg-stone-950/55 p-2.5">
                    <div className="grid grid-cols-[3rem_1fr_auto] gap-2.5">
                      {avatar ? (<img src={avatar} alt="" className="h-12 w-12 rounded-xl border border-cyan-300/20 bg-stone-900 object-cover" referrerPolicy="no-referrer" onError={imgerr}/>) : r.icon ? (<img src={r.icon} alt="" className="h-12 w-12 rounded-xl border border-white/10 bg-black p-2.5 object-contain" referrerPolicy="no-referrer" onError={imgerr}/>) : (<div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/10 bg-black font-mono text-xs text-cyan-200">{r.source.slice(0, 2).toLowerCase()}</div>)}
                      <div className="min-w-0">
                    ...
                    {!!fields.length && (<div className="mt-2 grid grid-cols-2 gap-1.5">
                        {fields.map(([key, val]) => (<div key={`${r.source}-${key}-${val}`} className="rounded-lg border border-white/5 bg-black/35 px-2 py-1.5">
                    ...
                    {!!extras.length && (<div className="mt-2 flex flex-wrap gap-1.5">
                        {extras.map(([key, val]) => (<span key={`${r.source}-${key}-${val}`} className="max-w-full truncate rounded-full border border-white/5 bg-black/35 px-2 py-1 font-mono text-[0.54rem] text-stone-300" title={`${key}: ${val}`}>
        ...
        <section className="rounded-xl border border-white/5 bg-black/25 p-2.5">
          ...
          <div className="space-y-1.5">
            {source_cards.slice(0, 10).length ? source_cards.slice(0, 10).map(r => (<a key={`${r.source}-${r.url}`} href={r.url} target="_blank" rel="noopener noreferrer" className="grid grid-cols-[1rem_1fr_auto] items-center gap-2 rounded-lg border border-white/5 bg-stone-950/50 px-2 py-1.5 hover:border-cyan-300/25 hover:bg-cyan-300/5">
              ...
              </a>)) : <p className="rounded-lg border border-white/5 bg-stone-950/50 px-2 py-4 text-center text-xs text-stone-600">no matching public entities found in this run</p>}
          </div>
        </section>
      </div>
    </article>);
}
function ReconModePanel({ onClose, onFoundCount, onLocations, stacked = false }) {
    const [query, setQuery] = (0, react_1.useState)("");
    const [runq, setRunq] = (0, react_1.useState)("");
    const [running, setRunning] = (0, react_1.useState)(false);
    const [fetches, setFetches] = (0, react_1.useState)([]);
    const [found, setFound] = (0, react_1.useState)([]);
    const [locations, setLocations] = (0, react_1.useState)([]);
    const [counts, setCounts] = (0, react_1.useState)(init_stats);
    const [show_found_entities, set_show_found_entities] = (0, react_1.useState)(true);
    const [err, setErr] = (0, react_1.useState)("");
    const evref = (0, react_1.useRef)(null);
    const georef = (0, react_1.useRef)(new Set());
    const stop = () => {
        evref.current?.close();
        evref.current = null;
        setRunning(false);
    ...
    (0, react_1.useEffect)(() => { onFoundCount?.(found.length); }, [found.length, onFoundCount]);
    (0, react_1.useEffect)(() => { onLocations?.(locations); }, [locations, onLocations]);
    const geocode = async (r) => {
        const loc = typeof r.info?.location === "string" ? r.info.location.trim() : "";
        if (!loc)
            return;
        const key = `${r.source.toLowerCase()}|${loc.toLowerCase()}`;
        if (georef.current.has(key))
            return;
        georef.current.add(key);
        try {
            const res = await fetch(`/api/recon/geocode?location=${encodeURIComponent(loc)}`, { cache: "no-store" });
            const geo = await res.json();
            if (!res.ok || !geo.found || !Number.isFinite(geo.lat) || !Number.isFinite(geo.lon))
                return;
            const p = {
    ...
    const begin = (e) => {
        ...
        es.addEventListener("progress", (ev) => setCounts(JSON.parse(ev.data)));
        es.addEventListener("done", (ev) => {
            setCounts(JSON.parse(ev.data));
            stop();
        });
        es.addEventListener("error", (ev) => {
            try {
                setErr(JSON.parse(ev.data).error || "stream failed");
            }
            catch {
                setErr("stream failed");
            }
            stop();
        });
        es.onerror = () => {
            if (running)
                setErr("stream interrupted");
            stop();
        };
    };
    const rate = (0, react_1.useMemo)(() => counts.total ? Math.round((counts.checked / counts.total) * 100) : 0, [counts]);
    const hero = (0, react_1.useMemo)(() => pickpfp(found), [found]);
    ...
    return (<section className={`pointer-events-auto fixed ${stacked ? "left-[53rem]" : "left-[28rem]"} top-[5.5rem] z-30 flex max-h-[calc(100svh-7rem)] w-[34rem] flex-col overflow-hidden rounded-2xl border border-white/10 bg-stone-950/90 text-stone-200 shadow-[0_20px_80px_rgba(0,0,0,0.65)] backdrop-blur-xl ${running ? "recon-search-active" : ""}`}>
      <div className="relative overflow-hidden border-b border-white/10 bg-stone-950/80 px-4 py-3">
        {running && (<div className="pointer-events-none absolute inset-0 opacity-80">
        ...
        <div className="mt-3 flex items-center gap-3 rounded-xl border border-white/10 bg-black/35 p-2.5">
          {hero ? (<img src={hero.avatar} alt="" className="h-14 w-14 shrink-0 rounded-xl border border-cyan-300/25 bg-stone-900 object-cover shadow-[0_0_22px_rgba(34,211,238,0.16)]" referrerPolicy="no-referrer" onError={imgerr}/>) : (<div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-stone-950 font-mono text-[0.58rem] uppercase tracking-widest text-stone-600">pfp</div>)}
        ...
        <form onSubmit={begin} className="mt-2 flex items-center gap-2">
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="username or email" className="h-10 flex-1 rounded-xl border border-white/10 bg-black/40 px-3 font-mono text-sm text-white outline-none placeholder:text-stone-600 focus:border-cyan-300/50"/>
        ...
        {running && (<div className="recon-signal-stack mt-3 rounded-xl border border-cyan-300/15 bg-black/35 p-2.5">
            ...
            <div className="mt-2 grid grid-cols-4 gap-1.5">
              {livebits.map((x, i) => (<div key={x} className="recon-packet-lane rounded-lg border border-white/5 bg-stone-950/70 px-2 py-1.5 font-mono text-[0.54rem] text-stone-300" style={{ animationDelay: `${i * 110}ms` }}>
                  {x}
                </div>))}
            </div>
          </div>)}
        ...
        <div className="mt-3 grid grid-cols-5 gap-2 text-center">
          {[
            ...
            ["error", counts.error],
            ["done", `${rate}%`],
        ].map(([k, v]) => (<div key={k} className={`rounded-xl border border-white/5 bg-black/30 px-2 py-2 ${running ? "recon-live-stat" : ""}`}>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        ...
        <section className="mb-3 overflow-hidden rounded-xl border border-white/5 bg-black/20">
          <div className="flex items-center justify-between gap-3 border-b border-white/5 px-3 py-2">
            ...
            <button type="button" aria-expanded={show_found_entities} aria-controls="recon-found-entities" onClick={() => set_show_found_entities(x => !x)} className="found-entities-toggle group flex shrink-0 items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 font-mono text-[0.55rem] font-bold uppercase tracking-[0.14em] text-cyan-100 hover:border-cyan-200/50 hover:bg-cyan-300/15">
          ...
          <button type="button" aria-label={show_found_entities ? "collapse found entities" : "expand found entities"} onClick={() => set_show_found_entities(x => !x)} className="block w-full border-b border-white/5 bg-black/25 px-3 py-1.5 hover:bg-cyan-300/5">
            <span className="mx-auto block h-1 w-16 rounded-full bg-stone-700/80"/>
          </button>

          {show_found_entities ? (<div id="recon-found-entities" className="space-y-2 p-3">
              {!found.length && (<div className="rounded-xl border border-white/5 bg-black/25 px-4 py-7 text-center text-sm text-stone-500">
              ...
              {[...found].sort((a, b) => source_quality(b) - source_quality(a) || pfprank(a) - pfprank(b) || a.source.localeCompare(b.source)).map((r) => {
                ...
                const fields = [["status", r.status ?? "-"], ["elapsed", `${r.elapsed_ms} ms`], ...profile_fields(r)].filter(([, v]) => v !== undefined && v !== null && v !== "");
                const others = clean_detail_entries(info);
                return (<article key={`${r.source}-${r.url}`} className="rounded-xl border border-white/10 bg-black/35 p-3">
                    <div className="flex gap-3">
                      {avatar ? (<img src={avatar} alt="" className="h-14 w-14 shrink-0 rounded-xl border border-white/10 bg-stone-900 object-cover" referrerPolicy="no-referrer" onError={imgerr}/>) : r.icon ? (<img src={r.icon} alt="" className="h-14 w-14 shrink-0 rounded-xl border border-white/10 bg-stone-950 p-3 object-contain" referrerPolicy="no-referrer" onError={imgerr}/>) : (<div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-stone-900 font-mono text-sm text-cyan-200">{r.source.slice(0, 2).toLowerCase()}</div>)}
                      <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-1.5">
                      {fields.map(([k, v]) => (<div key={k} className="flex items-center justify-between gap-2 rounded-lg border border-white/5 bg-stone-950/60 px-2 py-1.5">
                    ...
                    {!!others.length && (<div className="mt-2 rounded-lg border border-white/5 bg-stone-950/40 p-2">
                        {others.map(([k, v]) => (<div key={k} className="flex items-start justify-between gap-3 border-b border-white/5 py-1 last:border-0">
            ...
            </div>) : (<div className="found-entities-hidden grid grid-cols-[1fr_auto] items-center gap-3 px-3 py-3">
              <div className="min-w-0">
      ...
      <div className="h-44 border-t border-white/10 bg-black/35 p-3">
        ...
        <div className="h-[7.8rem] overflow-auto rounded-xl border border-white/5 bg-black/30">
          {fetches.map((f) => (<div key={f.index} className={`grid grid-cols-[1.25rem_1fr_auto] gap-2 border-b border-white/5 px-2.5 py-1.5 last:border-0 ${running && f.state === "checking" ? "recon-fetch-active" : ""}`}>
              {f.icon ? (<img src={f.icon} alt="" className="mt-0.5 h-4 w-4 rounded bg-stone-950 object-contain" referrerPolicy="no-referrer" onError={imgerr}/>) : (<span className="mt-0.5 h-4 w-4 rounded border border-white/5 bg-stone-900"/>)}
              <div className="min-w-0">
