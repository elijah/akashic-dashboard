...
exports.FlightMap = void 0;
const react_1 = require("react");
const FlightMap = ({ events, onClose }) => {
    (0, react_1.useEffect)(() => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 sm:p-10 backdrop-blur-sm">
      <div className="relative w-full h-full max-w-5xl bg-[#0a0a0a] border border-[#333] rounded-lg shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 w-full h-full p-4 overflow-y-auto bg-[#0a0a0a] flex flex-col gap-4 font-mono text-xs">
          ...
          {events.length === 0 ? (<div className="text-stone-500 text-center py-10">No active telemetry available.</div>) : events.map(e => (<div key={e.id} className="grid grid-cols-12 gap-4 bg-[#111] border border-[#222] rounded p-3 hover:border-blue-500/50 transition-colors items-center">
