...
exports.LiveStreamDeck = void 0;
const react_1 = require("react");
const lucide_react_1 = require("lucide-react");
const channel = (id, name, region, handle, video_id, hls_url, fallback_only = false) => ({ id, name, region, handle, video_id, hls_url, fallback_only });
const base_channels = [
...
const live_stream_deck = ({ onClose }) => {
    const [custom, setCustom] = (0, react_1.useState)([]);
    const [favorites, setFavorites] = (0, react_1.useState)([]);
    const [active_id, setActiveId] = (0, react_1.useState)(base_channels[0].id);
    const [region, setRegion] = (0, react_1.useState)("all");
    const [query, setQuery] = (0, react_1.useState)("");
    const [video_id, setVideoId] = (0, react_1.useState)(null);
    const [hls_url, setHlsUrl] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)("");
    const [muted, setMuted] = (0, react_1.useState)(true);
    const [playing, setPlaying] = (0, react_1.useState)(true);
    const [retry, setRetry] = (0, react_1.useState)(0);
    const [show_add, setShowAdd] = (0, react_1.useState)(false);
    const [new_name, setNewName] = (0, react_1.useState)("");
    const [new_source, setNewSource] = (0, react_1.useState)("");
    const iframe_ref = (0, react_1.useRef)(null);
    const video_ref = (0, react_1.useRef)(null);
    const channels = (0, react_1.useMemo)(() => [...base_channels, ...custom], [custom]);
    const active = channels.find(item => item.id === active_id) || channels[0];
    const filtered = (0, react_1.useMemo)(() => channels.filter(item => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (active.fallback_only) {
            ...
            setLoading(false);
            if (!active.video_id && !active.hls_url)
                setError("no playable source configured");
            return;
        }
        if (!active.handle) {
            setVideoId(active.video_id || null);
        ...
        fetch(`/api/youtube/live?channel=${encodeURIComponent(active.handle)}`)
            ...
            if (!found && !data.hlsUrl && !active.hls_url)
                setError(data.channelExists === false ? "channel unavailable" : "stream offline; no fallback available");
        })
            .catch(() => {
            if (!alive)
                return;
            ...
            setHlsUrl(active.hls_url || null);
            if (!active.video_id && !active.hls_url)
                setError("live detection unavailable");
        })
    ...
    const toggle_favorite = (id) => setFavorites(list => list.includes(id) ? list.filter(item => item !== id) : [...list, id]);
    const add_custom = () => {
        ...
        const item = { id, name, region: "custom", handle, video_id: watch, hls_url: hls, fallback_only: !!watch || !!hls, custom: true };
    ...
    const source_url = video_id
        ...
        : active?.handle
            ...
            : hls_url || "#";
    return (<div className="fixed inset-0 z-50 bg-black/90 p-2 sm:p-6 backdrop-blur-md">
      <div className="w-full h-full max-w-[1600px] mx-auto bg-[#0a0a0a] border border-[#333] rounded-md shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#222] bg-[#111] shrink-0">
          ...
          <div className="flex items-center gap-1">
            <button title={playing ? "pause" : "play"} onClick={() => setPlaying(value => !value)} className="p-2 text-stone-400 hover:text-white hover:bg-[#222] rounded">
              {playing ? <lucide_react_1.Pause size={16}/> : <lucide_react_1.Play size={16}/>}
            </button>
            <button title={muted ? "unmute" : "mute"} onClick={() => setMuted(value => !value)} className="p-2 text-stone-400 hover:text-white hover:bg-[#222] rounded">
              {muted ? <lucide_react_1.VolumeX size={16}/> : <lucide_react_1.Volume2 size={16}/>}
            </button>
        ...
        <div className="grid grid-cols-1 grid-rows-[45vh_1fr] lg:grid-rows-1 lg:grid-cols-[1fr_360px] flex-1 min-h-0 w-full h-full overflow-hidden">
          <div className="bg-black relative min-h-[40vh] lg:min-h-0 flex items-center justify-center border-b lg:border-b-0 border-[#222]">
            ...
            {!loading && error && (<div className="flex flex-col items-center gap-3 text-center px-6 z-10">
                ...
                <button onClick={() => setRetry(value => value + 1)} className="flex items-center gap-2 border border-red-500/40 text-red-400 px-3 py-2 text-[10px] uppercase hover:bg-red-500/10">
                  <lucide_react_1.RefreshCw size={13}/> retry
                </button>
              </div>)}
            {!loading && !error && video_id && (<iframe ref={iframe_ref} key={video_id} src={`https://www.youtube.com/embed/${video_id}?autoplay=1&mute=${muted ? 1 : 0}&enablejsapi=1&rel=0`} className="w-full h-full absolute inset-0" title={`${active?.name} live stream`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>)}
            {!loading && !error && !video_id && hls_url && (<video ref={video_ref} key={hls_url} src={hls_url} className="w-full h-full absolute inset-0 object-contain" autoPlay muted={muted} controls onError={() => setError("direct hls stream is unavailable or restricted in this browser")}/>)}
            {!loading && !error && !video_id && !hls_url && (<div className="text-stone-600 text-xs uppercase tracking-widest z-10">no video source</div>)}
          </div>
          ...
          <aside className="bg-[#111] flex flex-col min-h-[300px] lg:min-h-0 border-t lg:border-t-0 lg:border-l border-[#222] overflow-hidden">
            <div className="p-3 border-b border-[#222] flex flex-col gap-2 shrink-0">
              <div className="flex gap-2">
                <label className="flex-1 flex items-center gap-2 bg-[#0a0a0a] border border-[#333] px-2 rounded-sm">
                  <lucide_react_1.Search size={13} className="text-stone-500"/>
              ...
              <div className="flex gap-1 overflow-x-auto pb-1 custom-scrollbar">
                {regionz.map(item => (<button key={item} onClick={() => setRegion(item)} className={`px-2 py-1 text-[8px] uppercase whitespace-nowrap border ${region === item ? "border-red-500/50 bg-red-500/10 text-red-400" : "border-[#333] text-stone-500 hover:text-stone-300"} rounded-sm transition-colors`}>
                    {item}
                  </button>))}
              </div>
              {show_add && (<div className="grid gap-2 border-t border-[#222] pt-2">
            ...
            <div className="flex-1 overflow-y-auto p-2 grid gap-1 content-start custom-scrollbar">
              {filtered.map(item => (<div key={item.id} className={`flex items-center border transition-colors rounded-sm ${active?.id === item.id ? "bg-[#222] border-red-500/50" : "border-[#2b2b2b] hover:border-stone-500 bg-[#0a0a0a]"}`}>
                  ...
                  <button title="favorite" onClick={() => toggle_favorite(item.id)} className={`p-2 ${favorites.includes(item.id) ? "text-yellow-400" : "text-stone-600 hover:text-stone-300"} transition-colors`}>
                    <lucide_react_1.Star size={14} fill={favorites.includes(item.id) ? "currentColor" : "none"}/>
                  </button>
                  {item.custom && (<button title="remove custom channel" onClick={() => remove_custom(item.id)} className="p-2 text-stone-600 hover:text-red-400 transition-colors">
...
exports.LiveStreamDeck = live_stream_deck;
