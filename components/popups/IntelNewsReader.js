...
exports.IntelNewsReader = void 0;
const react_1 = require("react");
const IntelNewsReader = ({ title, events, onClose }) => {
    (0, react_1.useEffect)(() => {
        document.body.style.overflow = "hidden";
        return () => { document.body.style.overflow = "auto"; };
    }, []);
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 sm:p-10 backdrop-blur-sm">
      <div className="relative w-full h-full max-w-4xl bg-[#0a0a0a] border border-[#333] rounded-lg shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 w-full h-full p-4 overflow-y-auto bg-[#0a0a0a] flex flex-col gap-4 font-mono">
          {events.length === 0 ? (<div className="text-stone-500 text-center py-10">No intelligence intercepts available for this sector.</div>) : events.map(e => (<div key={e.id} className="bg-[#111] border border-[#222] rounded p-4 flex flex-col gap-2 hover:border-[#444] transition-colors">
               ...
               {e.source_url && (<a href={e.source_url} target="_blank" rel="noreferrer" className="text-[10px] text-blue-400 hover:text-blue-300 mt-2 flex items-center gap-1 w-fit">
                   Verify Source ↗
                 </a>)}
            </div>))}
        </div>
...
exports.IntelNewsReader = IntelNewsReader;
