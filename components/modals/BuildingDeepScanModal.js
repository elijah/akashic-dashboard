...
exports.default = BuildingDeepScanModal;
const react_1 = require("react");
function BuildingDeepScanModal({ url, onClose }) {
    ...
    return (<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <div className="relative flex h-[90vh] w-full max-w-6xl flex-col rounded-3xl border border-white/10 bg-stone-950 shadow-2xl overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10">
                    {loading && (<div className="flex h-full flex-col items-center justify-center gap-4">
                    ...
                    {error && !loading && (<div className="flex h-full flex-col items-center justify-center">
                    ...
                    {data && !loading && (<div className="flex flex-col gap-8">
                            {data.warning && (<div className="rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3 text-xs leading-relaxed text-amber-100">
                                    {data.warning}
                                </div>)}

                            
                            {data.cvuOutline && (<div className="flex flex-col md:flex-row gap-6 rounded-2xl border border-emerald-500/20 bg-emerald-950/10 p-6">
                                    <div className="flex w-full md:w-1/3 flex-col justify-center gap-4">
                                        ...
                                        {data.cvuFacts && Object.entries(data.cvuFacts).map(([key, val], idx) => (<div key={key} className="flex flex-col border-l-2 border-emerald-500/40 pl-4">
                            ...
                            {data.drawings && data.drawings.length > 0 && (<div>
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        {data.drawings.map((img, i) => (<a key={img} href={img} target="_blank" rel="noopener noreferrer" className="overflow-hidden rounded-xl border border-emerald-500/20 bg-white/[0.03]">
                            ...
                            <div className="flex flex-col lg:flex-row gap-8">
                                ...
                                <div className="w-full lg:w-1/3 shrink-0">
                                    ...
                                    {Object.keys(data.facts).length > 0 ? (<div className="flex flex-col gap-1 rounded-xl border border-white/5 bg-white/[0.02] p-1">
                                            {Object.entries(data.facts).map(([key, val]) => (<div key={key} className="flex items-start justify-between rounded-lg px-3 py-2 hover:bg-white/5 transition-colors">
                                        ...
                                        </div>) : (<p className="text-sm text-stone-500">No structured metrics extracted.</p>)}
                                </div>
                                ...
                                <div className="w-full lg:w-2/3">
                                    ...
                                    {data.images.length > 0 ? (<div className="columns-1 sm:columns-2 gap-4 space-y-4">
                                            {data.images.map((img, i) => (<div key={i} className="break-inside-avoid rounded-xl border border-white/10 bg-black/50 overflow-hidden relative group">
                                        ...
                                        </div>) : (<div className="rounded-xl border border-dashed border-white/10 flex h-48 items-center justify-center text-center p-6">
