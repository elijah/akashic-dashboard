...
exports.DEFAULT_WM_LAYERS = void 0;
exports.default = WorldMonitorDeck;
const react_1 = require("react");
exports.DEFAULT_WM_LAYERS = {
    conflicts: false,
    wars: false,
    civil_unrest: false,
    violence: false,
...
function WorldMonitorDeck({ visible, layers: st, onToggle, counts, has_correlations }) {
    const [exp, set_exp] = (0, react_1.useState)(true);
    const [search, set_search] = (0, react_1.useState)("");
    ...
    if (!visible)
        return null;
    if (!exp)
        return (<button onClick={() => set_exp(true)} className="pointer-events-auto h-12 w-12 rounded-xl border border-red-500/30 bg-stone-950/90 text-red-400 backdrop-blur-xl hover:bg-stone-900/90">
    ...
    return (<div ref={ref} className="w-[300px] pointer-events-auto overflow-hidden rounded-xl border border-stone-800 bg-stone-950/95 font-mono text-stone-300 shadow-2xl backdrop-blur-xl">
      ...
      <div className="px-2 pt-2">
        <input type="text" value={search} onChange={e => set_search(e.target.value)} placeholder="search layers" className="w-full rounded border border-stone-700 bg-stone-900/80 px-2 py-1.5 text-xs text-stone-300 outline-none placeholder:text-stone-600 focus:border-cyan-500/50"/>
      </div>
      <div className="flex max-h-[60vh] flex-col gap-1 overflow-y-auto p-2">
        {groups.map(g => (<div key={g}>
            ...
            {filtered.filter(l => l.group === g).map(l => {
                ...
                return (<label key={l.id} className={`flex cursor-pointer items-center gap-3 rounded border p-2 transition-colors ${on ? "border-stone-700 bg-stone-800/60" : "border-transparent hover:bg-stone-900/50"}`}>
