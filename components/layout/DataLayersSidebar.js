...
exports.default = DataLayersSidebar;
...
function DataLayersSidebar({ layers, onToggle, radarCount, usgsCount, radioCount, sigintCount, gdacsCount, airqCount, reconCount, gilCount }) {
    const [cat, setCat] = (0, react_1.useState)("ALL");
    const toggle = (id) => onToggle(id);
    const filtered = cat === "ALL" ? LAYERS : LAYERS.filter((l) => l.category === cat);
    const activeCount = Object.values(layers).filter(Boolean).length;
    return (<div className="panel-flat flex flex-col h-full bg-m3-surface text-m3-on-surface border-r border-m3-outline-variant/40 w-72 rounded-none border-t-0 border-b-0 border-l-0 shadow-lg relative z-10">
            ...
            <div className="px-4 pb-4 border-b border-m3-outline-variant/30">
                <div className="flex p-1 bg-m3-surface-container rounded-lg border border-m3-outline-variant/20 shadow-inner">
                    {CATS.map((c) => (<button key={c} onClick={() => setCat(c)} className={`flex-1 py-1.5 text-[0.55rem] font-semibold tracking-widest rounded-md transition-all duration-200 cursor-pointer ${cat === c
            ...
            const displayCount = layer.id === "radar" && radarCount !== undefined
                ? radarCount
                                ...
                                : layer.id === "airq" && airqCount !== undefined
                                    ...
                                    : layer.id === "recon" && reconCount !== undefined
                                        ...
                                        : layer.id === "gil" && gilCount !== undefined
                                            ? gilCount
                                            : 0;
            return (<div key={layer.id} onClick={() => {
                    if (canToggle)
                        toggle(layer.id);
                                ...
                                <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className={`text-[0.45rem] font-bold px-1.5 py-0.5 rounded uppercase tracking-widest ${on ? "bg-m3-on-surface text-m3-surface" : "bg-m3-surface-container-high text-m3-on-surface-variant"}`}>
                                                {layer.category}
                                            </span>
                    ...
                    e.stopPropagation();
                                        ...
                                        <label className="toggle">
                                            <input type="checkbox" checked={on} disabled={!canToggle} onChange={() => {
                    if (canToggle)
                        toggle(layer.id);
                }}/>
