...
if (typeof window !== 'undefined') {
    ...
    setInterval(() => {
        ...
        if (ds.alerts.length > 200)
            ds.alerts.pop();
        if (ds.feed.length > 200)
            ds.feed.pop();
        if (ds.iocs.length > 200)
            ds.iocs.pop();
        if (ds.vessels.length > 200)
            ds.vessels.pop();
        if (ds.evts.length > 200)
            ds.evts.pop();
        if (ds.sanc.length > 200)
            ds.sanc.pop();
        ping_subs();
    }, 400);
}
const use_ds = () => {
    const [, f] = (0, react_1.useState)({});
    (0, react_1.useEffect)(() => {
        const c = () => f({});
        sub_cb.push(c);
...
const alerts_mod = () => (<div className="flex flex-col gap-2 p-2 h-full overflow-y-auto">
    {use_ds().alerts.map((a) => (<div key={a.id} className={`text-xs border-l-2 p-1.5 bg-[#111] ${a.lvl === "high" ? "border-rose-500 text-rose-300" : a.lvl === "med" ? "border-amber-500 text-amber-300" : "border-blue-500 text-blue-300"}`}>
        <span className="opacity-50 mr-2">[{a.time}]</span> {a.msg}
      </div>))}
  </div>);
const timeline_mod = () => (<div className="flex gap-4 p-2 h-full items-center overflow-x-auto whitespace-nowrap">
    {use_ds().evts.slice(0, 50).map((e) => (<div key={e.id} className="flex flex-col gap-1 min-w-[120px]">
...
const watchlist_mod = () => (<div className="flex flex-col p-2 h-full overflow-y-auto gap-1 text-xs">
    {use_ds().vessels.map((v, i) => (<div key={i} className="flex justify-between items-center p-1 hover:bg-[#1a1a1a] cursor-pointer border-b border-[#222]">
...
const cyber_mod = () => (<div className="flex flex-col p-2 h-full overflow-y-auto gap-1 text-[10px] font-mono">
    <div className="text-emerald-500 mb-1 sticky top-0 bg-black/80 backdrop-blur pb-1">:: active indicators of compromise</div>
    {use_ds().iocs.map((v, i) => (<div key={i} className="flex gap-2 text-[#777] border-b border-[#111]">
        <span className="text-rose-500 w-16">[{v.hits} hits]</span>
        <span className="text-[#ccc]">{v.ip}</span>
      </div>))}
  </div>);
const infra_mod = () => (<div className="grid grid-cols-3 gap-2 p-2 h-full items-center">
    {['suez', 'panama', 'hormuz'].map((c, i) => (<div key={i} className="border border-[#222] p-2 flex flex-col items-center justify-center bg-[#0a0a0a]">
...
const risk_mod = () => {
    ...
    return (<div className="flex p-4 h-full items-center justify-center gap-6">
...
const feed_mod = () => (<div className="flex flex-col h-full overflow-y-auto p-2 gap-1 text-[10px]">
    {use_ds().feed.map((f, i) => (<div key={i} className="text-[#444] font-mono whitespace-nowrap">
        {f}
      </div>))}
  </div>);
const entity_card_mod = () => {
    const s = use_ds().sanc[0] || { ent: 'N/A', match: 0 };
    return (<div className="flex flex-col p-4 h-full gap-4 text-xs">
...
const claim_table_mod = () => (<div className="flex flex-col h-full">
    ...
    <div className="flex flex-col overflow-y-auto p-2 gap-1">
      {use_ds().sanc.slice(0, 30).map((s) => (<div key={s.id} className="flex text-xs items-center hover:bg-[#111] p-1 border-b border-[#111] transition-colors cursor-pointer">
...
const render_mod = (type, children) => {
    switch (type) {
        ...
        default:
            return (<div className="absolute inset-0 flex items-center justify-center opacity-30">
          <span className="text-[10px] text-[#555] tracking-widest uppercase">{type} MODULE</span>
        </div>);
    }
};
function DashboardWorkspace({ children, onClose }) {
    const [active_template, set_active_template] = (0, react_1.useState)('global_crisis');
    const [menu_open, set_menu_open] = (0, react_1.useState)(false);
    const template = TEMPLATES.find(t => t.id === active_template) || TEMPLATES[0];
    return (<div className="absolute inset-0 z-[100] pointer-events-none flex flex-col font-mono bg-black">
      ...
      <div className="h-12 bg-[#050505]/90 backdrop-blur border-b border-[#333] flex items-center px-4 justify-between pointer-events-auto">
        <div className="flex items-center gap-4">
          <button onClick={() => set_menu_open(!menu_open)} className="text-xs font-bold text-emerald-500 hover:text-emerald-400 border border-emerald-900/50 hover:border-emerald-500 px-3 py-1 rounded bg-[#111]">
            [ MENU ]
          </button>
          {onClose && (<button onClick={onClose} className="text-xs font-bold text-rose-500 hover:text-rose-400 border border-rose-900/50 hover:border-rose-500 px-3 py-1 rounded bg-[#111]">
              [ EXIT ]
            </button>)}
          <div className="flex flex-col">
      ...
      <framer_motion_1.AnimatePresence>
        {menu_open && (<framer_motion_1.motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-12 left-4 w-80 bg-[#0a0a0a]/95 backdrop-blur-md border border-[#333] shadow-[0_20px_50px_rgba(0,0,0,0.8)] z-50 pointer-events-auto rounded flex flex-col p-2 gap-2">
            <div className="text-[10px] text-[#666] uppercase tracking-widest mb-2 px-2 border-b border-[#222] pb-2">Workspace Templates</div>
            {TEMPLATES.map(t => (<button key={t.id} onClick={() => { set_active_template(t.id); set_menu_open(false); }} className={`text-left px-3 py-2 text-xs font-bold tracking-widest rounded transition-colors ${active_template === t.id ? 'bg-emerald-900/20 text-emerald-400 border border-emerald-900/50' : 'text-[#888] hover:text-white hover:bg-[#111] border border-transparent'}`}>
                {t.name}
              </button>))}
      ...
      <div className="flex-1 p-4 pointer-events-auto overflow-hidden flex flex-col gap-4">
        ...
        <div className="flex-1 grid grid-cols-12 gap-4 auto-rows-fr">
          {template.panels.map(p => (<div key={p.id} className={`bg-[#050505]/80 backdrop-blur border border-[#333] rounded overflow-hidden flex flex-col`} style={{ gridColumn: `span ${p.w}`, gridRow: `span ${p.h}` }}>
