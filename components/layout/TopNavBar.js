...
function TopNavBar() {
    ...
    const [localTime, setLocalTime] = (0, react_1.useState)("");
    const [tz, setTz] = (0, react_1.useState)("");
    (0, react_1.useEffect)(() => {
    ...
    return (<div className="flex items-center w-full flex-shrink-0 h-[52px] bg-m3-surface border-b border-m3-outline-variant">
            ...
            <nav className="flex items-center h-full px-1">
                {NAV_ITEMS.map((item) => (<button key={item.label} className={`h-full px-[14px] text-[0.75rem] bg-transparent border-none cursor-pointer outline-none transition-all duration-150 whitespace-nowrap border-b-[3px] ${item.active ? "font-medium text-m3-on-surface border-m3-primary" : "font-normal text-m3-on-surface-variant border-transparent hover:text-m3-on-surface hover:bg-m3-surface-container"}`}>
                        {item.label}
                    </button>))}
            </nav>
            ...
            <div className="flex items-center gap-5 px-5 flex-shrink-0 h-full border-l border-m3-outline-variant">
                {SYS_STATUS.map((s) => (<div key={s.label} className="flex items-center gap-2">
