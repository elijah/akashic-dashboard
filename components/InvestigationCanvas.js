...
exports.InvestigationCanvas = void 0;
const react_1 = require("react");
const react_draggable_1 = __importDefault(require("react-draggable"));
const InvestigationCanvas = ({ onClose }) => {
    const [invs, setInvs] = (0, react_1.useState)([]);
    const [activeInv, setActiveInv] = (0, react_1.useState)(null);
    const [veracity, setVeracity] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [mode, setMode] = (0, react_1.useState)("workspace");
      ...
      <div className="relative w-[95vw] h-[95vh] bg-[#050505] border border-cyan-900/50 shadow-[0_0_80px_rgba(8,145,178,0.15)] flex flex-col pointer-events-auto overflow-hidden font-mono text-stone-300 rounded-xl">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-[320px] bg-[#0a0a0a] border-r border-cyan-900/30 flex flex-col shrink-0">
            {mode === "workspace" && (<>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  {loading && !activeInv ? (<div className="text-[10px] text-stone-600 text-center py-4 uppercase">SYNCING DB...</div>) : invs.length === 0 ? (<div className="text-[10px] text-stone-600 text-center py-4 uppercase">NO ACTIVE CASES</div>) : invs.map(inv => (<div key={inv.id} onClick={() => load_inv(inv.id)} className={`p-2 rounded border cursor-pointer transition-all ${activeInv?.id === inv.id ? 'bg-[#111] border-cyan-500 shadow-[inset_2px_0_0_#06b6d4]' : 'bg-[#0f0f0f] border-stone-800 hover:border-stone-600'}`}>
            ...
            {mode === "disinfo" && (<>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  {loading ? (<div className="text-[10px] text-stone-600 text-center py-4 uppercase animate-pulse">RUNNING BAYESIAN SWEEP...</div>) : campaigns.length === 0 ? (<div className="text-[10px] text-green-600 text-center py-4 uppercase">NO CAMPAIGNS DETECTED (48H)</div>) : campaigns.map((c, i) => (<div key={i} className="p-2 bg-[#111] border border-red-900/50 rounded flex flex-col gap-1">
          ...
          <div className="flex-1 bg-[#050505] relative overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#0a0a0a] to-[#000]">
            ...
            {loading && activeInv && (<div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="text-xs font-bold text-cyan-500 uppercase tracking-widest animate-pulse">RECONSTRUCTING TIMELINE...</div>
              </div>)}

            {!activeInv && mode === "workspace" && (<div className="absolute inset-0 flex items-center justify-center">
            ...
            {activeInv && (<div className="relative w-full h-full p-4 overflow-auto">
                ...
                <div className="relative w-[3000px] h-[3000px]">
                  ...
                  {activeInv.events?.map((ev, i) => (<react_draggable_1.default key={ev.event_id} defaultPosition={{ x: 350, y: 50 + (i * 150) }}>
                  ...
                  {activeInv.claims?.map((cl, i) => {
                const claim = cl.claim;
                const ver = veracity?.find((v) => v.claim_id === claim.id);
                const conf = ver ? Math.round(ver.posterior_prob * 100) : Math.round(claim.confidence);
                let color = "text-yellow-500";
                let bcolor = "border-yellow-500/50";
                if (conf > 75) {
                    color = "text-green-500";
                    bcolor = "border-green-500/50";
                }
                if (conf < 30) {
                    color = "text-red-500";
                    bcolor = "border-red-500/50";
                }
                return (<react_draggable_1.default key={cl.claim_id} defaultPosition={{ x: 700 + (i % 2) * 300, y: 100 + Math.floor(i / 2) * 160 }}>
                        <div className={`absolute w-[260px] bg-[#0a0a0a] border ${bcolor} rounded shadow-xl cursor-move flex flex-col z-10`}>
                          ...
                          {ver && ver.contradicts.length > 0 && (<div className="px-2 pb-2">
                  ...
                  {activeInv.entities?.map((en, i) => (<react_draggable_1.default key={en.entity_id} defaultPosition={{ x: 50 + (i * 100), y: 400 + (i * 50) }}>
            ...
            {mode === "disinfo" && !loading && campaigns.length > 0 && (<div className="w-full h-full flex items-center justify-center">
...
exports.InvestigationCanvas = InvestigationCanvas;
