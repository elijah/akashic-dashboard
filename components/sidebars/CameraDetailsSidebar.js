...
exports.default = CameraDetailsSidebar;
...
function CameraDetailsSidebar({ camera, onClose }) {
    if (!camera)
        return null;
    ...
    const viewer = camera.embedUrl ? (<iframe src={camera.embedUrl} title={camera.name} className="h-full w-full rounded-card border border-m3-outline/40 bg-black" loading="lazy" referrerPolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-popups"/>) : camera.thumbnailUrl ? (<img src={camera.thumbnailUrl} alt={camera.name} className="h-full w-full rounded-card border border-m3-outline/40 bg-black object-contain"/>) : (<div className="flex h-full w-full items-center justify-center rounded-card border border-m3-outline/40 bg-black text-[0.7rem] uppercase tracking-widest text-m3-on-surface-variant">
            No live preview available
        </div>);
    return (<div className="fixed inset-0 z-[220] flex cursor-default items-center justify-center bg-black/70 p-4 text-m3-on-surface backdrop-blur-sm sm:p-6">
                ...
                <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 lg:grid-cols-[minmax(0,1fr)_320px]">
                    ...
                    <div className="overflow-y-auto border-t border-m3-outline-variant bg-m3-surface lg:border-l lg:border-t-0">
                        ...
                        <div className="px-3.5 py-3">
                            {camera.pageUrl && (<a href={camera.pageUrl} target="_blank" rel="noopener noreferrer" className="inline-flex w-full items-center justify-center rounded-card border border-cyan-400/40 bg-cyan-400/10 px-3 py-2 text-[0.62rem] font-semibold uppercase tracking-wider text-cyan-200 hover:bg-cyan-400/15">
                                    Open Camera Source
                                </a>)}
