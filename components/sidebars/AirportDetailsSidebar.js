...
exports.default = AirportDetailsSidebar;
const react_1 = require("react");
function AirportDetailsSidebar({ airport, onClose }) {
    ...
    if (!airport)
        return null;
    return (<div className="bg-m3-surface border border-m3-outline-variant shadow-panel-sm rounded-panel w-[320px] text-m3-on-surface animate-in fade-in slide-in-from-right duration-300">
            ...
            <div className="px-4 py-5">
                ...
                {loading && !details ? (<div className="font-mono text-[0.6rem] text-m3-on-surface-variant uppercase">FETCHING INTELLIGENCE...</div>) : (<>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {details?.stats?.topAirlines?.map((a) => (<span key={a} className="font-mono text-[0.6rem] text-m3-on-surface bg-m3-surface-container px-2 py-1 rounded-tag border border-m3-outline">
                                        {a}
                                    </span>))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-col border border-m3-outline rounded-card overflow-hidden">
                                {details?.liveNearby?.length > 0 ? details.liveNearby.map((f, i) => (<div key={f.icao24} className={`grid grid-cols-3 gap-2 p-2.5 bg-m3-surface-container ${i > 0 ? 'border-t border-m3-outline' : ''}`}>
                                    ...
                                    </div>)) : (<div className="p-4 bg-m3-surface-container text-center font-mono text-[0.6rem] text-m3-on-surface-variant uppercase">
