...
exports.default = CityDetailsSidebar;
...
function compactDensity(n) {
    if (n >= 1e6)
        return (n / 1e6).toFixed(1) + "M/km^2";
    if (n >= 1e3)
        return (n / 1e3).toFixed(1) + "K/km^2";
    return Math.round(n).toLocaleString("en-US") + "/km^2";
}
function CityDetailsSidebar({ city, onClose, worldMonitorActive = false, geoIntel, onCameraSelect }) {
    const [liveData, setLiveData] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [cams, setCams] = (0, react_1.useState)([]);
    const [prevCityKey, setPrevCityKey] = (0, react_1.useState)("");
    // Violence data state
    const [violenceData, setViolenceData] = (0, react_1.useState)(null);
    const [violenceLoading, setViolenceLoading] = (0, react_1.useState)(false);
    const [violenceExpanded, setViolenceExpanded] = (0, react_1.useState)(true);
    const [prevViolenceKey, setPrevViolenceKey] = (0, react_1.useState)("");
    (0, react_1.useEffect)(() => {
        if (!city)
            return;
        const key = city.name + "|" + city.iso;
        if (key === prevCityKey && liveData)
            return;
        setPrevCityKey(key);
        setLoading(true);
        setLiveData(null);
        ...
        fetch(`/api/city/${encodeURIComponent(city.name)}?country=${encodeURIComponent(city.country)}&iso=${encodeURIComponent(city.iso)}`, { signal: ctrl.signal })
            ...
            console.error("[CityPanel] fetch error:", err);
            setLoading(false);
        } });
        fetch("/markers.json")
            .then(r => r.json())
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (key === prevViolenceKey && violenceData)
            return;
        ...
        fetch(`/api/city-violence/${encodeURIComponent(city.name)}`, { signal: ctrl.signal })
            .then(r => r.json())
            .then((d) => { setViolenceData(d); setViolenceLoading(false); })
            .catch(err => { if (err.name !== "AbortError") {
            console.error("[CityPanel] violence fetch error:", err);
            setViolenceLoading(false);
        } });
        return () => ctrl.abort();
    ...
    const unrest = (0, live_deck_core_1.build_city_unrest)(geoIntel?.events || [], city);
    ...
    const protest_svg = () => (<svg viewBox="0 0 48 48" className="h-11 w-11" fill="none" aria-label="protest activity">
    ...
    const row = (label, value) => (<div key={label} className="flex items-center justify-between px-3.5 py-1.5">
    ...
    const section = (title, children) => (<div className="border-b border-m3-outline-variant">
    ...
    return (<div className="max-h-[88vh] w-[300px] overflow-y-auto rounded-panel border border-m3-outline-variant bg-m3-surface text-m3-on-surface shadow-panel-sm">
            ...
            {liveData?.wikidata?.imageUrl && (<div className="px-3 pb-1 pt-3">
            ...
            {loading && (<div className="px-3.5 py-4 text-center">
            ...
            <div className="border-b border-m3-outline-variant px-3 py-3">
                <div className={`rounded-card border p-3 ${unrest_style.border} ${unrest_style.bg}`}>
                    ...
                    {unrest.events.length > 0 ? (<div className="mt-3 space-y-1.5 border-t border-white/10 pt-2">
                            {unrest.events.slice(0, 3).map(event => (<div key={event.id} className="flex gap-2 text-[0.48rem]">
                        ...
                        </div>) : (<div className="mt-2 border-t border-white/10 pt-2 text-[0.46rem] text-m3-on-surface-variant">No nearby sourced civil-unrest events in the current feed.</div>)}
                </div>
            </div>
            ...
            {section("Verified Facts", <>
                ...
                {liveData?.wikidata?.officialWebsite && (<div className="flex items-center justify-between px-3.5 py-1.5">
                        ...
                        <a href={liveData.wikidata.officialWebsite} target="_blank" rel="noopener noreferrer" className="max-w-[145px] truncate font-mono text-[0.55rem] text-blue-400 hover:underline">
                            {(() => { try {
                return new URL(liveData.wikidata.officialWebsite).hostname;
            }
            catch {
                return liveData.wikidata.officialWebsite;
            } })()}
                        </a>
                    </div>)}
            </>)}
            ...
            {worldMonitorActive && (<div className="border-b border-m3-outline-variant">
                    <button onClick={() => setViolenceExpanded(!violenceExpanded)} className="w-full cursor-pointer bg-transparent border-none outline-none">
                        <div className="px-3.5 pb-1 pt-2.5 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                {violenceData && riskStyle && (<span className={`px-1.5 py-0.5 rounded text-[0.4rem] font-bold tracking-wider ${riskStyle.bg} ${riskStyle.border} ${riskStyle.text} border`}>
                                        {riskStyle.label}
                                    </span>)}
                            </div>
                            <svg width="10" height="10" viewBox="0 0 10 10" fill="none" className={`text-m3-on-surface-variant transition-transform duration-200 ${violenceExpanded ? "rotate-180" : ""}`}>
                                <path d="M2 4L5 7L8 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </div>
                    </button>

                    {violenceExpanded && (<div className="px-3.5 py-2">
                            {violenceLoading && (<div className="py-3 text-center">
                            ...
                            {violenceData && (<>
                                    ...
                                    {riskStyle && (<div className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg mb-3 ${riskStyle.bg} border ${riskStyle.border}`}>
                        ...
                        return (<div key={incident.id} className={`rounded-lg px-2.5 py-2 border transition-all hover:brightness-110 ${sev.bg} ${sev.border}`}>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        {incident.fatalities > 0 && (<span className="text-[0.45rem] font-mono font-bold text-red-400">
                                                                {incident.fatalities} KIA
                                                            </span>)}
                                                        {incident.injuries > 0 && (<span className="text-[0.45rem] font-mono font-bold text-amber-400">
                            ...
                            {!violenceLoading && !violenceData && (<p className="text-[0.55rem] text-m3-on-surface-variant py-2">
            ...
            <div className="border-b border-m3-outline-variant">
                ...
                <div className="px-3.5 py-2">
                    {liveData?.wiki?.extract ? (<>
                            ...
                            {liveData.wiki.pageUrl && (<a href={liveData.wiki.pageUrl} target="_blank" rel="noopener noreferrer" className="mt-1 inline-block text-[0.5rem] text-blue-400 hover:underline">
            ...
            <div className="border-b border-m3-outline-variant">
                ...
                <div className="px-3.5 py-2 flex flex-col gap-2">
                    {cams.length > 0 ? cams.map((c, i) => (<button key={c.id + i} type="button" onClick={() => onCameraSelect?.(markerToCamera(c))} className="rounded-card border border-m3-outline/40 overflow-hidden bg-black/50 text-left transition-colors hover:border-cyan-400/50 hover:bg-cyan-400/5">
                        ...
                        </button>)) : (<p className="text-[0.55rem] text-m3-on-surface-variant">No public cameras found nearby.</p>)}
                </div>
            </div>
