...
exports.default = QuakeDetailsSidebar;
const fmt = (v, d = 2) => v === null || v === undefined || !Number.isFinite(v) ? "n/a" : v.toFixed(d);
const whole = (v) => v === null || v === undefined || !Number.isFinite(v) ? "n/a" : Math.round(v).toLocaleString();
function row(label, value) {
    return (<div key={label} className="flex justify-between gap-3 px-3.5 py-1.5">
...
function QuakeDetailsSidebar({ quake, onClose }) {
    if (!quake)
        return null;
