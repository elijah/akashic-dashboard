...
exports.default = radio_details_sidebar;
...
const origin_icon = (value) => {
    try {
        return value ? `${new URL(value).origin}/favicon.ico` : null;
    }
    catch {
        return null;
    }
...
function row(label, value, tone = "text-m3-on-surface") {
    return (<div key={label} className="flex min-h-8 items-center justify-between gap-3 border-b border-m3-outline-variant/50 px-3.5 py-1.5 last:border-0">
...
function radio_details_sidebar({ radio, onClose }) {
    const [meta, set_meta] = (0, react_1.useState)(null);
    const [playing, set_playing] = (0, react_1.useState)(false);
    const [connecting, set_connecting] = (0, react_1.useState)(false);
    const [ready, set_ready] = (0, react_1.useState)(false);
    const [audio_error, set_audio_error] = (0, react_1.useState)(null);
    const [volume, set_volume] = (0, react_1.useState)(0.72);
    const [icon_index, set_icon_index] = (0, react_1.useState)(0);
    const audio_ref = (0, react_1.useRef)(null);
    const metadata_ctrl_ref = (0, react_1.useRef)(null);
    const trying_ref = (0, react_1.useRef)(false);
    (0, react_1.useEffect)(() => {
        if (!radio?.streamUrl) {
            set_meta(null);
            return;
        ...
        return () => {
            ctrl.abort();
            if (metadata_ctrl_ref.current === ctrl)
                metadata_ctrl_ref.current = null;
    ...
    const audio_src = (0, react_1.useMemo)(() => radio?.streamUrl ? `/api/radio/stream?url=${encodeURIComponent(radio.streamUrl)}` : "", [radio?.streamUrl]);
    ...
    const icon_urls = (0, react_1.useMemo)(() => {
        if (!radio)
            return [];
    ...
    const media_error = () => {
        ...
        if (code === 2)
            return "network interrupted - retry available";
        if (code === 3)
            return "station audio could not be decoded";
        if (code === 4)
            return "station format is unsupported";
        return "stream connection failed";
    };
    const play_once = async (audio, source, attempt, proxied) => {
        audio.pause();
    ...
    const toggle_play = async () => {
        ...
        const sources = Array.from(new Set([radio.streamUrl, meta?.stream?.final_url].filter((x) => !!x)));
    ...
    if (!radio)
        return null;
    return (<div className="flight-sidebar max-h-[88vh] w-[340px] overflow-y-auto rounded-panel border border-m3-outline-variant bg-m3-surface text-m3-on-surface shadow-panel-sm">
            <audio ref={audio_ref} preload="none" src={audio_src} onCanPlay={() => set_ready(true)} onPlaying={() => { set_connecting(false); set_playing(true); }} onPause={() => set_playing(false)} onStalled={() => { if (!trying_ref.current)
        set_audio_error("station stalled - press play to reconnect"); }} onError={() => {
            if (trying_ref.current)
                return;
            set_audio_error(media_error());
            set_connecting(false);
            set_playing(false);
            ...
            <div className="relative border-b border-m3-outline-variant bg-m3-surface-container-low px-3.5 py-3">
                <div className="flex items-center gap-3 pr-7">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-card border border-m3-outline bg-m3-surface-container">
                        {icon_urls[icon_index] ? (<img src={icon_urls[icon_index]} alt="" className="h-8 w-8 object-contain" onError={() => set_icon_index(x => x + 1)}/>) : <lucide_react_1.RadioTower size={20} className="text-blue"/>}
                    </div>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                    ...
                    {secure && <span className="inline-flex items-center gap-1 rounded-tag border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-[0.48rem] text-emerald-400"><lucide_react_1.ShieldCheck size={9}/> secure</span>}
                </div>
            </div>
            ...
            <div className="border-b border-m3-outline-variant px-3.5 py-3.5">
                ...
                <div className="mt-3 flex h-10 items-center gap-3 border-t border-m3-outline-variant/60 pt-3">
                    <button type="button" onClick={toggle_play} title={playing ? "pause" : "play"} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-blue/50 bg-blue/10 text-blue hover:bg-blue/20">
                        {playing ? <lucide_react_1.Pause size={15} fill="currentColor"/> : <lucide_react_1.Play size={15} fill="currentColor" className={connecting ? "ml-0.5 animate-pulse" : "ml-0.5"}/>}
                    </button>
                    <div className="flex h-7 flex-1 items-end gap-[3px] overflow-hidden" aria-hidden="true">
                        {[9, 17, 12, 23, 15, 26, 11, 20, 14, 24, 10, 18].map((h, i) => (<span key={i} className={`w-full rounded-sm bg-blue/70 ${playing ? "animate-pulse" : "opacity-25"}`} style={{ height: playing ? h : 4, animationDelay: `${i * 70}ms` }}/>))}
                    </div>
                    <lucide_react_1.Volume2 size={14} className="shrink-0 text-m3-on-surface-variant"/>
                    <input aria-label="volume" type="range" min="0" max="1" step="0.05" value={volume} onChange={e => {
            const val = Number(e.target.value);
            set_volume(val);
            if (audio_ref.current)
                audio_ref.current.volume = val;
            ...
            {(homepage || station_url || socials.length > 0) && (<div className="px-3.5 py-3">
                    <div className="mb-2 text-[0.48rem] font-semibold uppercase tracking-[0.16em] text-m3-primary">source links</div>
                    <div className="flex flex-wrap gap-2">
                        {homepage && <a href={homepage} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-tag border border-m3-outline bg-m3-surface-container px-2.5 py-1.5 text-[0.54rem] text-m3-on-surface hover:border-blue/50"><lucide_react_1.Globe2 size={11}/> station site</a>}
                        {station_url && <a href={station_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-tag border border-m3-outline bg-m3-surface-container px-2.5 py-1.5 text-[0.54rem] text-m3-on-surface hover:border-blue/50"><lucide_react_1.RadioTower size={11}/> radio garden</a>}
                        {socials.map(link => <a key={`${link.platform}-${link.url}`} href={link.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-tag border border-m3-outline px-2.5 py-1.5 text-[0.52rem] text-m3-on-surface-variant hover:text-m3-on-surface">{link.platform}<lucide_react_1.ExternalLink size={9}/></a>)}
                    </div>
                </div>)}
        </div>);
}
