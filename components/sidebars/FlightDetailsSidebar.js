...
exports.default = FlightDetailsSidebar;
const react_1 = require("react");
function FlightDetailsSidebar({ flight, onClose }) {
    ...
    let anomalyScore = 0;
    if (details?.squawk === "7700")
        anomalies.push("EMERGENCY SQUAWK 7700");
    else if (details?.squawk === "7600")
        anomalies.push("RADIO FAILURE 7600");
    else if (details?.squawk === "7500")
        anomalies.push("HIJACKING 7500");
    else if (details?.squawk === "0000" || details?.squawk === "7777")
        anomalies.push("MILITARY INTERCEPT / INVALID SQUAWK");
    if (alt_ft > 45000 && flight.category !== "MILITARY")
        anomalies.push("UNUSUAL CIVILIAN ALTITUDE (>45k ft)");
    if (spd_kts > 600 && flight.category !== "MILITARY")
        anomalies.push("OVERSPEED WARNING (>600kts)");
    if (flight.category === "MILITARY" || details?.operator.toLowerCase().includes("air force") || details?.operator.toLowerCase().includes("navy")) {
        isMilitary = true;
        if (alt_ft > 15000 && alt_ft < 30000 && spd_kts < 300)
            anomalies.push("POSSIBLE ISR ORBIT DETECTED");
        if (alt_ft > 20000 && spd_kts > 400 && spd_kts < 500)
            anomalies.push("POSSIBLE AERIAL REFUELING ORBIT");
    }
    ...
    if (ageSeconds > 300)
        anomalies.push(`STALE ADS-B (${Math.round(ageSeconds / 60)}m gap) - POSS DARK FLIGHT`);
    anomalyScore = Math.min(100, anomalies.length * 35);
    const row = (label, value) => (<div key={label} className="flex justify-between items-center px-3.5 py-1.5">
    ...
    return (<div className="bg-m3-surface border border-m3-outline-variant shadow-panel-sm rounded-panel flight-sidebar w-[280px] max-h-[88vh] overflow-y-auto text-m3-on-surface">
            ...
            {details?.photoUrl && (<div className="relative border-b border-m3-outline-variant bg-m3-surface-container">
            ...
            {anomalies.length > 0 && (<div className="p-3 bg-orange-900/20 border-b border-orange-900/50">
                    ...
                    <div className="flex flex-col gap-1">
                        {anomalies.map((a, i) => (<div key={i} className="text-[0.55rem] font-mono text-orange-200 border-l-2 border-orange-500 pl-2 py-0.5">{a}</div>))}
                    </div>
                </div>)}

            
            {details && (details.origin || details.destination) && (<div className="px-3.5 py-2 bg-m3-surface-container flex justify-between items-center gap-1.5 border-b border-m3-outline-variant">
            ...
            <div className="py-2">
                {loading && !details ? (<div className="p-3.5 text-[0.55rem] text-m3-on-surface-variant text-center font-mono">QUERYING INTELLIGENCE SOURCES...</div>) : details ? (<>
