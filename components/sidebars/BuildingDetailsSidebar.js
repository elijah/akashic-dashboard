...
exports.default = BuildingDetailsSidebar;
const react_1 = require("react");
const BuildingDeepScanModal_1 = __importDefault(require("@/components/modals/BuildingDeepScanModal"));
function BuildingDetailsSidebar({ buildingCoord, onClose }) {
    const [data, setData] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [prevKey, setPrevKey] = (0, react_1.useState)("");
    const [deepScanUrl, setDeepScanUrl] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        if (!buildingCoord)
            return;
        ...
        fetch(`/api/wm/building?lat=${buildingCoord.lat}&lon=${buildingCoord.lon}`, { signal: ctrl.signal })
            ...
            console.error("[BuildingPanel] error:", err);
            setLoading(false);
        } });
        return () => ctrl.abort();
    }, [buildingCoord]);
    if (!buildingCoord)
        return null;
    const row = (label, value) => (<div key={label} className="flex items-center justify-between px-3.5 py-1.5 border-b border-m3-outline-variant/50 last:border-0">
    ...
    return (<div className="max-h-[88vh] w-[300px] overflow-y-auto rounded-panel border border-m3-outline-variant bg-m3-surface text-m3-on-surface shadow-panel-sm relative">
            ...
            {loading && (<div className="px-3.5 py-6 flex flex-col items-center justify-center gap-2">
            ...
            {!loading && data && !data.found && (<div className="px-3.5 py-4 text-center">
            ...
            {!loading && data && data.found && (<>
                    {data.wikiImage && (<div className="px-3 pb-1 pt-3">
                    ...
                    {section("Entity Details", <>
                        ...
                        {data.structuralDetails?.website && (<div className="flex items-center justify-between px-1 py-1.5 border-b border-m3-outline-variant/50 last:border-0">
                                ...
                                <a href={data.structuralDetails.website} target="_blank" rel="noopener noreferrer" className="max-w-[145px] truncate font-mono text-[0.55rem] text-blue-400 hover:underline">
                                    {(() => { try {
                    return new URL(data.structuralDetails.website).hostname;
                }
                catch {
                    return data.structuralDetails.website;
                } })()}
                                </a>
                            </div>)}
                    ...
                    {data.description && (<div className="border-b border-m3-outline-variant">
                            <div className="px-3.5 pb-1 pt-2.5">
                    ...
                    {data.webDiscoveries && data.webDiscoveries.length > 0 && section("OSINT Web Discovery", <>
                        {data.webDiscoveries.map((discovery, idx) => (<div key={idx} className="flex flex-col gap-1 px-1 py-2 border-b border-m3-outline-variant/50 last:border-0">
                                ...
                                <span className="font-mono text-[0.45rem] text-m3-on-surface-variant opacity-60 truncate">
                                    {(() => { try {
                    return new URL(discovery.link).hostname;
                }
                catch {
                    ...
                    {data.rankedDeepScanUrl && (<div className="px-3.5 py-4 border-b border-m3-outline-variant">
                            <button onClick={() => data.rankedDeepScanSource === "archdaily search"
                    ? window.open(data.rankedDeepScanUrl, "_blank", "noopener,noreferrer")
                    : setDeepScanUrl(data.rankedDeepScanUrl)} className="w-full rounded bg-emerald-500/20 px-4 py-2.5 font-mono text-[0.65rem] font-bold tracking-widest text-emerald-400 transition-colors hover:bg-emerald-500/30 flex justify-center items-center gap-2 border border-emerald-500/40">
            ...
            {deepScanUrl && (<BuildingDeepScanModal_1.default url={deepScanUrl} onClose={() => setDeepScanUrl(null)}/>)}
        </div>);
}
