...
exports.default = SatelliteDetailsSidebar;
const react_1 = require("react");
const satellite = __importStar(require("satellite.js"));
const MU = 398600.4418;
const EARTH_RADIUS_KM = 6378.137;
const fmt = (v, digits = 2) => v === null || !Number.isFinite(v) ? "n/a" : v.toFixed(digits);
const whole = (v) => v === null || !Number.isFinite(v) ? "n/a" : Math.round(v).toLocaleString();
const pickNum = (txt) => {
...
function parseEpoch(line1) {
    ...
    if (!Number.isFinite(yr) || !Number.isFinite(day))
        return null;
...
const row = (label, value) => (<div key={label} className="flex justify-between items-start gap-3 px-3.5 py-1.5">
...
function SatelliteDetailsSidebar({ satellite: sat, onClose }) {
    const [intel, setIntel] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        if (!sat) {
            setIntel(null);
            return;
    ...
    return (<div className="bg-m3-surface border border-m3-outline-variant shadow-panel-sm rounded-panel flight-sidebar w-[340px] max-h-[88vh] overflow-y-auto text-m3-on-surface">
            <div className={`px-3.5 py-3 relative border-b ${isMilitary ? 'border-red-900/50 bg-red-900/10' : 'border-m3-outline-variant'}`}>
                ...
                {(sat.operator || sat.purpose) && (<div className="mt-2 space-y-0.5">
            ...
            {spaceRisk.length > 0 && (<div className="p-3 bg-indigo-900/20 border-b border-indigo-900/50">
                    ...
                    <div className="flex flex-col gap-1">
                        {spaceRisk.map((a, i) => (<div key={i} className="text-[0.55rem] font-mono text-indigo-200 border-l-2 border-indigo-500 pl-2 py-0.5">{a}</div>))}
                    </div>
                </div>)}
