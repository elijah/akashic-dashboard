...
exports.default = CountryDashboard;
const react_1 = require("react");
const live_deck_core_1 = require("@/lib/geo-intelligence/live-deck-core");
const ParliamentChart_1 = __importDefault(require("./ParliamentChart"));
const TABS = ["OVERVIEW", "GEOGRAPHY", "SOCIETY", "GOVERNMENT", "ECONOMY", "ENERGY", "INFRA", "SECURITY", "GEO-INTEL"];
function CountryDashboard({ iso, geoIntel, onClose }) {
    const [data, setData] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [tab, setTab] = (0, react_1.useState)("OVERVIEW");
    const [local_geo, set_local_geo] = (0, react_1.useState)(null);
    ...
    (0, react_1.useEffect)(() => {
        ...
        fetch("/api/geo-intelligence", { cache: "no-store" })
            .then(r => {
            if (!r.ok)
                throw new Error("geo-intelligence unavailable");
            return r.json();
        })
            .then(feed => { if (active)
            set_local_geo(feed); })
    ...
    const row = (label, value, accent) => (<div className="flex justify-between items-start py-1.5 px-1 border-b border-m3-outline-variant/30 last:border-0 gap-4">
    ...
    const textBlock = (title, text, truncateLines) => {
        if (!text)
            return null;
        return (<div className="pt-2 pb-1 border-b border-m3-outline-variant/30 last:border-0 relative group">
                <div className="font-sans text-[0.55rem] font-semibold text-m3-primary uppercase tracking-widest mb-1 px-1 flex justify-between items-center">
                    {title}
                    {truncateLines && (<button onClick={() => setPopup({ title, content: <div className="text-[0.65rem] font-mono leading-relaxed text-justify whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: text }}/> })} className="text-[0.5rem] bg-m3-surface-variant text-m3-on-surface-variant px-2 py-0.5 rounded cursor-pointer hover:bg-m3-primary hover:text-m3-on-primary transition-colors shrink-0">
                            READ FULL
                        </button>)}
                </div>
    ...
    const pill = (text, color) => (<span key={text} className={`text-[0.5rem] font-mono px-2 py-0.5 rounded-tag border ${color || "text-m3-on-surface bg-m3-surface-container border-m3-outline"}`}>{text}</span>);
    const formatGdp = (val) => {
        if (!val)
            return "N/A";
        if (val >= 1e12)
    ...
    const country_live = data ? (0, live_deck_core_1.build_country_intel)(active_geo || { events: [] }, { name: data.name, iso2: data.iso2, iso3: data.iso3 }, data.latestNews || []) : null;
    const energy_live = (0, live_deck_core_1.build_country_domain_summary)(country_live?.events || [], "energy");
    const infra_live = (0, live_deck_core_1.build_country_domain_summary)(country_live?.events || [], "infrastructure");
    const security_live = (0, live_deck_core_1.build_country_domain_summary)(country_live?.events || [], "security");
    const join_html = (...parts) => parts.filter(Boolean).join("<br/><br/>");
    const tab_counts = {
        ENERGY: energy_live.count,
        INFRA: infra_live.count,
        SECURITY: security_live.count,
    };
    const render_domain_live = (title, summary, tone) => {
        const colors = {
            amber: {
        ...
        return (<div className={`mb-4 border ${colors.border} bg-m3-surface-container/70 p-3 rounded-card`}>
            ...
            ].map(([label, value, color]) => (<div key={label} className="border border-m3-outline-variant/50 bg-m3-surface px-2 py-1.5 text-center rounded-card">
                ...
                <div className="space-y-1">
                    {summary.events.length ? summary.events.slice(0, 5).map(event => (<a key={event.id} href={event.source_url || "#"} target={event.source_url ? "_blank" : undefined} rel="noreferrer" className="block border-l-2 border-m3-outline bg-m3-surface px-2 py-1.5 hover:bg-m3-surface-variant">
                        ...
                        </a>)) : (<div className="py-2 text-center font-mono text-[0.5rem] text-m3-on-surface-variant">
    ...
    const renderOverview = () => {
        ...
        return (<div className="space-y-4">
            ...
            ].map(m => (<div key={m.label} className="bg-m3-surface-container border border-m3-outline p-2 rounded-card text-center">
                ...
                {country_live && (<div className="space-y-2 border-y border-m3-outline-variant/40 py-3">
                    ...
                    ["critical", country_live.critical, "text-red-400"],
                ].map(([label, value, color]) => (<div key={label} className="rounded-card border border-m3-outline bg-m3-surface-container p-2 text-center">
                        ...
                        {country_live.latest_news.slice(0, 3).map(news => (<a key={news.id} href={news.source_url || "#"} target={news.source_url ? "_blank" : undefined} rel="noreferrer" className="block border-l-2 border-cyan-500/50 bg-m3-surface-container px-2 py-1.5 hover:bg-m3-surface-variant">
    ...
    const renderGeography = () => {
        if (!data?.factbook?.Geography)
            return <div className="text-center py-8 text-[0.6rem] text-m3-on-surface-variant">NO DATA</div>;
    ...
    const renderSociety = () => {
        if (!data?.factbook?.['People and Society'])
            return <div className="text-center py-8 text-[0.6rem] text-m3-on-surface-variant">NO DATA</div>;
    ...
    const renderGovernment = () => {
        if (!data?.factbook?.Government)
            return <div className="text-center py-8 text-[0.6rem] text-m3-on-surface-variant">CLASSIFIED / UNAVAILABLE</div>;
        const gov = data.factbook.Government;
        const openChamberPopup = (name, title, chamber) => {
            if (!chamber)
                return;
            const textContent = `<strong>Name:</strong> ${chamber['chamber name']?.text || chamber['legislature name']?.text || 'Unknown'}<br/>` +
            ...
            setPopup({
                ...
                content: (<div className="space-y-4">
                        ...
                        <ParliamentChart_1.default text={chamber['parties elected and seats per party']?.text || chamber['election results']?.text || ''}/>
                    </div>)
            });
        };
        ...
        return (<div className="space-y-2">
                ...
                <div className="space-y-2 mb-3">
                    ...
                    {hasLegacyAssembly && (<button onClick={() => openChamberPopup("Legislative", "Legislative", gov['Legislative branch'])} className="w-full bg-m3-primary/10 border border-m3-primary/30 p-3 rounded-card text-left hover:bg-m3-primary/20 transition-colors">
                    ...
                    {hasSingleChamber && (<button onClick={() => openChamberPopup("National", "National", gov['Legislative branch - single chamber'])} className="w-full bg-m3-primary/10 border border-m3-primary/30 p-3 rounded-card text-left hover:bg-m3-primary/20 transition-colors">
                    ...
                    {hasLowerChamber && (<button onClick={() => openChamberPopup("Lower", "Lower", gov['Legislative branch - lower chamber'])} className="w-full bg-m3-primary/10 border border-m3-primary/30 p-3 rounded-card text-left hover:bg-m3-primary/20 transition-colors">
                    ...
                    {hasUpperChamber && (<button onClick={() => openChamberPopup("Upper", "Upper", gov['Legislative branch - upper chamber'])} className="w-full bg-m3-primary/10 border border-m3-primary/30 p-3 rounded-card text-left hover:bg-m3-primary/20 transition-colors">
    ...
    const renderEconomy = () => {
        ...
        return (<div className="space-y-1">
                ...
                {e && (<div className="mt-4 space-y-2 border-t border-m3-outline-variant/30 pt-4">
    ...
    const renderEnergy = () => {
        ...
        ].filter((entry) => Boolean(entry[1])).map(([label, value]) => `${label}: ${value}`).join("<br/>");
        return (<div className="space-y-2">
                {render_domain_live("LIVE ENERGY SIGNALS", energy_live, "amber")}
                {active_geo?.energy_prices?.length ? (<div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {active_geo.energy_prices.slice(0, 6).map(market => (<div key={market.id} className="border border-m3-outline-variant/50 bg-m3-surface-container p-2 rounded-card">
    ...
    const renderSecurity = () => {
        ...
        return (<div className="space-y-2">
                ...
                {m && (<div className="mt-4 space-y-2 border-t border-m3-outline-variant/30 pt-4">
                ...
                {t && (<div className="mt-4 space-y-2 border-t border-m3-outline-variant/30 pt-4">
    ...
    const renderGeoIntel = () => {
        ...
        return (<div className="space-y-3 font-mono">
                ...
                {active_geo.correlations.length > 0 && (<div className="bg-purple-900/20 border border-purple-800/40 p-2 rounded">
                ...
                <div className="bg-stone-900 p-2 border border-stone-800 rounded">
                    ...
                    <div className="space-y-1">
                        {active_geo.events.slice(0, 10).map(e => (<div key={e.id} className="text-[0.55rem] flex gap-2">
    ...
    const render_country_intel = () => {
        ...
        const correlations = (active_geo?.correlations || []).filter(item => item.event_ids.some(id => event_ids.has(id)) || item.categories.some(category => categoryz.has(category)));
        return (<div className="space-y-4 font-mono">
            ...
            ].map(([label, value, color]) => (<div key={label} className="rounded-card border border-stone-800 bg-stone-900 p-2 text-center">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                        {domainz.length ? domainz.map(([domain, count]) => (<div key={domain} className="flex items-center justify-between rounded border border-stone-800 bg-stone-900 px-2 py-1.5">
                            ...
                            </div>)) : (<div className="col-span-2 py-2 text-center text-[0.5rem] text-stone-600">No active country-scoped domains.</div>)}
                    </div>
                </div>

                {correlations.length > 0 && (<div>
                        ...
                        <div className="space-y-1.5">
                            {correlations.slice(0, 4).map(item => (<div key={item.id} className="rounded border border-purple-800/40 bg-purple-900/20 p-2">
                                    <div className="flex items-center justify-between">
                ...
                <div>
                    ...
                    <div className="space-y-1.5">
                        {country_live.latest_news.length ? country_live.latest_news.map(news => (<a key={news.id} href={news.source_url || "#"} target={news.source_url ? "_blank" : undefined} rel="noreferrer" className="block rounded border border-stone-800 bg-stone-900 p-2 hover:border-cyan-700">
                            ...
                            </a>)) : <div className="py-2 text-center text-[0.5rem] text-stone-600">No current country news returned.</div>}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="space-y-1.5">
                        {country_live.events.length ? country_live.events.slice(0, 25).map(event => (<div key={event.id} className="rounded border border-stone-800 bg-stone-900 p-2">
                            ...
                            </div>)) : <div className="py-2 text-center text-[0.5rem] text-stone-600">No ongoing country-scoped events in the current feed.</div>}
                    </div>
                </div>

                {active_geo?.source_health && (<div className="border-t border-stone-800 pt-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                            {active_geo.source_health.source_groups.map(source => (<span key={source.name} className={`border px-1.5 py-0.5 text-[0.4rem] ${source.status === "live" ? "border-emerald-800 text-emerald-400" : source.status === "delayed" ? "border-amber-800 text-amber-400" : "border-red-800 text-red-400"}`}>
    ...
    return (<div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            ...
            <div className="relative flex max-h-[90vh] w-[680px] max-w-[94vw] flex-col overflow-hidden rounded-panel border border-m3-outline-variant bg-m3-surface text-m3-on-surface shadow-panel-sm pointer-events-auto animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-5 py-4 border-b border-m3-outline-variant relative shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {data && (<div className="text-[0.55rem] text-m3-on-surface-variant mt-0.5">
                                    {data.formalName}
                                </div>)}
                        </div>
                    </div>
                ...
                <div className="flex border-b border-m3-outline-variant bg-m3-surface-container/50 overflow-x-auto hide-scrollbar shrink-0">
                    {TABS.map(t => (<button key={t} onClick={() => setTab(t)} className={`flex-none px-3 py-2.5 text-[0.55rem] font-semibold uppercase tracking-wider border-b-2 transition-colors ${tab === t
                ...
                : "text-m3-on-surface-variant border-transparent hover:text-m3-on-surface hover:bg-m3-surface-container"}`}>
                            <span className="flex items-center gap-1.5">
                                {t}
                                {tab_counts[t] !== undefined ? (<span className="min-w-4 border border-current/25 bg-black/15 px-1 py-0.5 font-mono text-[0.42rem] leading-none">
                                        {tab_counts[t]}
                                    </span>) : null}
                            </span>
                        </button>))}
                ...
                <div className="relative flex-1 overflow-hidden">
                    <div className="px-5 py-4 overflow-y-auto h-full hide-scrollbar">
                        {loading ? (<div className="text-center py-8">
                            ...
                            </div>) : data ? (content[tab]()) : (<div className="text-center py-8">
                                <div className="font-mono text-[0.6rem] text-m3-on-surface-variant uppercase">CLASSIFIED / NO DATA</div>
                            </div>)}
                    </div>

                    
                    {popup && (<div className="absolute inset-0 z-50 bg-m3-surface flex flex-col animate-in slide-in-from-bottom-8 duration-200">
