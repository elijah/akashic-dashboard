...
const FlightDetailsSidebar_1 = __importDefault(require("@/components/sidebars/FlightDetailsSidebar"));
const AirportDetailsSidebar_1 = __importDefault(require("@/components/sidebars/AirportDetailsSidebar"));
const SatelliteDetailsSidebar_1 = __importDefault(require("@/components/sidebars/SatelliteDetailsSidebar"));
const QuakeDetailsSidebar_1 = __importDefault(require("@/components/sidebars/QuakeDetailsSidebar"));
const RadioDetailsSidebar_1 = __importDefault(require("@/components/sidebars/RadioDetailsSidebar"));
const CameraDetailsSidebar_1 = __importDefault(require("@/components/sidebars/CameraDetailsSidebar"));
const CityDetailsSidebar_1 = __importDefault(require("@/components/sidebars/CityDetailsSidebar"));
const IntelDetailsSidebar_1 = __importDefault(require("@/components/sidebars/IntelDetailsSidebar"));
const BuildingDetailsSidebar_1 = __importDefault(require("@/components/sidebars/BuildingDetailsSidebar"));
const ReconModePanel_1 = require("@/components/intelligence/ReconModePanel");
const CountryDashboard_1 = __importDefault(require("@/components/intelligence/CountryDashboard"));
const WorldMonitorDeck_1 = __importStar(require("@/components/layout/WorldMonitorDeck"));
const DashboardWorkspace_1 = require("@/components/layout/DashboardWorkspace");
const BottomIntelligenceDeck_1 = require("@/components/layout/BottomIntelligenceDeck");
const WeatherMapControls_1 = require("@/components/map/WeatherMapControls");
const map_core_1 = require("@/lib/weather/map-core");
const MapGlobe = (0, dynamic_1.default)(() => Promise.resolve().then(() => __importStar(require("@/components/map/MapGlobe"))), { ssr: false });
...
function satelliteImageFor(sat) {
    if (!sat)
        return "https://images.unsplash.com/photo-1446776811953-b23d57bd21aa?auto=format&fit=crop&w=960&q=80";
    const n = sat.name.toUpperCase();
    if (n.includes("STARLINK"))
        return "https://images.unsplash.com/photo-1517976487492-5750f3195933?auto=format&fit=crop&w=960&q=80";
    if (n.includes("ISS") || n.includes("ZARYA") || n.includes("TIANGONG"))
        return "https://images.unsplash.com/photo-1454789548928-9efd52dc4031?auto=format&fit=crop&w=960&q=80";
    if (sat.category === "weather")
        return "https://images.unsplash.com/photo-1508349937151-22b68b72d5b1?auto=format&fit=crop&w=960&q=80";
    return "https://images.unsplash.com/photo-1446776709462-d6b525c57bd3?auto=format&fit=crop&w=960&q=80";
}
function flightImageFor(flight) {
    if (!flight)
    ...
    if (flight.category === "MILITARY")
        return "https://images.unsplash.com/photo-1569629743817-70d8db6c323b?auto=format&fit=crop&w=960&q=80";
    if (flight.onGround)
        return "https://images.unsplash.com/photo-1504196606672-aef5c9cefc92?auto=format&fit=crop&w=960&q=80";
    return "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?auto=format&fit=crop&w=960&q=80";
}
function fmtCoord(v, lat = true) {
...
function nowUtcIsoShort(date) {
    if (!date)
        return "---- -- -- --:--:--Z";
    return date.toISOString().replace("T", " ").slice(0, 19) + "Z";
}
function Home() {
    const [activeStyle, setActiveStyle] = (0, react_1.useState)("standard");
    const [activeImagery, setActiveImagery] = (0, react_1.useState)("dark");
    const [isGlobe, setIsGlobe] = (0, react_1.useState)(false);
    const [dashboardMode, setDashboardMode] = (0, react_1.useState)(false);
    const [geoIntelData, setGeoIntelData] = (0, react_1.useState)(null);
    const [geoIntelLoading, setGeoIntelLoading] = (0, react_1.useState)(false);
    const [streetViewMode, setStreetViewMode] = (0, react_1.useState)(false);
    const [showImageryPicker, setShowImageryPicker] = (0, react_1.useState)(false);
    const [viewMetrics, setViewMetrics] = (0, react_1.useState)({ cameraKm: 27643, scaleKm: 5000 });
    const [enabledLayers, setEnabledLayers] = (0, react_1.useState)(DEFAULT_LAYERS);
    const [radarFilter, setRadarFilter] = (0, react_1.useState)("all");
    const [weatherMode, setWeatherMode] = (0, react_1.useState)(map_core_1.weather_default_mode);
    const [activeTab, setActiveTab] = (0, react_1.useState)("Overview");
    const [globeCommand, setGlobeCommand] = (0, react_1.useState)(null);
    const [clock, setClock] = (0, react_1.useState)(null);
    const [worldMonitorLayers, setWorldMonitorLayers] = (0, react_1.useState)(WorldMonitorDeck_1.DEFAULT_WM_LAYERS);
    const [wmCounts, setWmCounts] = (0, react_1.useState)({});
    const [isSearchOpen, setIsSearchOpen] = (0, react_1.useState)(false);
    const [searchQuery, setSearchQuery] = (0, react_1.useState)("");
    const [selectedFlight, setSelectedFlight] = (0, react_1.useState)(null);
    const [selectedAirport, setSelectedAirport] = (0, react_1.useState)(null);
    const [selectedSatellite, setSelectedSatellite] = (0, react_1.useState)(null);
    const [selectedQuake, setSelectedQuake] = (0, react_1.useState)(null);
    const [selectedRadio, setSelectedRadio] = (0, react_1.useState)(null);
    const [selectedCamera, setSelectedCamera] = (0, react_1.useState)(null);
    const [selectedIntel, setSelectedIntel] = (0, react_1.useState)(null);
    const [selectedSigint, setSelectedSigint] = (0, react_1.useState)(null);
    ...
    const [selectedBuilding, setSelectedBuilding] = (0, react_1.useState)(null);
    const [radarCount, setRadarCount] = (0, react_1.useState)(0);
    const [usgsCount, setUsgsCount] = (0, react_1.useState)(0);
    const [radioCount, setRadioCount] = (0, react_1.useState)(0);
    const [reconCount, setReconCount] = (0, react_1.useState)(0);
    const [reconLocations, setReconLocations] = (0, react_1.useState)([]);
    const [sigintCount, setSigintCount] = (0, react_1.useState)(0);
    const [gdacsCount, setGdacsCount] = (0, react_1.useState)(0);
    const [airqCount, setAirqCount] = (0, react_1.useState)(0);
    const [sigintData, setSigintData] = (0, react_1.useState)(null);
    const [sigintFilters, setSigintFilters] = (0, react_1.useState)({
        tab: "overview",
        severity: "low",
        confidence: 0,
        evidence: "all",
    ...
    const [gilCount, setGilCount] = (0, react_1.useState)(0);
    const [positionFix, setPositionFix] = (0, react_1.useState)(null);
    const nodeRefFlight = (0, react_1.useRef)(null);
    const nodeRefAirport = (0, react_1.useRef)(null);
    const nodeRefSatellite = (0, react_1.useRef)(null);
    const nodeRefQuake = (0, react_1.useRef)(null);
    const nodeRefRadio = (0, react_1.useRef)(null);
    const nodeRefIntel = (0, react_1.useRef)(null);
    const nodeRefSigint = (0, react_1.useRef)(null);
    const nodeRefCity = (0, react_1.useRef)(null);
    const nodeRefBuilding = (0, react_1.useRef)(null);
    const commandIdRef = (0, react_1.useRef)(1);
    (0, react_1.useEffect)(() => {
    ...
    const handleCitySelect = (0, react_1.useCallback)((city) => {
    ...
    const handleViewMetrics = (0, react_1.useCallback)((metrics) => {
        setViewMetrics((prev) => (prev.cameraKm === metrics.cameraKm && prev.scaleKm === metrics.scaleKm ? prev : metrics));
    }, []);
    const handleToggleLayer = (id) => {
        setEnabledLayers((prev) => {
            const next = { ...prev, [id]: !prev[id] };
            if (id === "gil" && !next["gil"]) {
                setWorldMonitorLayers(WorldMonitorDeck_1.DEFAULT_WM_LAYERS);
            }
            return next;
        });
    };
    const handleToggleWmLayer = (id) => {
    ...
    const runCommand = (type) => {
        setGlobeCommand({ id: commandIdRef.current++, type });
    };
    const applyTabPreset = (tab) => {
        setActiveTab(tab);
        if (tab === "Overview") {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (tleRes.status === 'fulfilled' && tleRes.value?.satellites) {
                const sat = tleRes.value.satellites.find((s) => s.name.toLowerCase().includes(q));
                if (sat) {
                    const tacticalSat = {
                        id: sat.id || sat.name,
                        name: sat.name,
            ...
            if (usgsRes.status === 'fulfilled' && usgsRes.value?.events) {
                const eq = usgsRes.value.events.find((e) => e.title?.toLowerCase().includes(q) || e.place?.toLowerCase().includes(q));
                if (eq) {
                    setSelectedQuake(eq);
    ...
    const layerGroups = (0, react_1.useMemo)(() => [
        {
            id: "airspace",
    ...
    const handleStreetViewPick = (0, react_1.useCallback)((_url) => { }, []);
    ...
    const satMetrics = (0, react_1.useMemo)(() => {
    ...
    const flightMetrics = (0, react_1.useMemo)(() => {
        if (!selectedFlight)
            return null;
    ...
    const intelHeader = (0, react_1.useMemo)(() => {
    ...
    const mapElement = (<MapGlobe isGlobe={isGlobe} activeStyle={activeStyle} activeImagery={activeImagery} streetViewMode={streetViewMode} enabledLayers={enabledLayers} radarFilter={radarFilter} reconLocations={reconLocations} weatherMode={weatherMode} sigintFilters={sigintFilters} uiCommand={globeCommand} onFlightSelect={setSelectedFlight} onAirportSelect={setSelectedAirport} onSatelliteSelect={setSelectedSatellite} onQuakeSelect={setSelectedQuake} onRadioSelect={setSelectedRadio} onCameraSelect={setSelectedCamera} onIntelSelect={setSelectedIntel} onSigintSelect={setSelectedSigint} onSigintData={setSigintData} onRadarCount={setRadarCount} onUsgsCount={setUsgsCount} onRadioCount={setRadioCount} onSigintCount={setSigintCount} onGdacsCount={setGdacsCount} onAirqCount={setAirqCount} onCitySelect={handleCitySelect} onCountrySelect={setSelectedCountry} onGilCount={setGilCount} onPositionFix={setPositionFix} onViewMetrics={handleViewMetrics} onStreetViewPick={handleStreetViewPick} worldMonitorLayers={worldMonitorLayers} onWmCounts={setWmCounts} onBuildingSelect={setSelectedBuilding}/>);
    if (dashboardMode) {
        return (<div className="relative min-h-screen w-full overflow-x-hidden bg-black text-stone-300">
                <DashboardWorkspace_1.DashboardWorkspace onClose={() => setDashboardMode(false)}>
                    {mapElement}
                </DashboardWorkspace_1.DashboardWorkspace>
            </div>);
    }
    return (<div className="relative min-h-screen w-full overflow-x-hidden bg-black text-stone-300">
            <div className="fixed inset-0 z-0">
                ...
                {enabledLayers.noaa === true && (<WeatherMapControls_1.WeatherMapControls mode={weatherMode} on_mode={setWeatherMode} on_close={() => setEnabledLayers((prev) => ({ ...prev, noaa: false }))}/>)}

                {enabledLayers.sigint === true && (<section className="pointer-events-auto fixed left-[28rem] top-[5.5rem] z-30 w-[24rem] rounded-2xl border border-cyan-400/15 bg-stone-950/85 p-4 text-stone-200 shadow-[0_20px_80px_rgba(0,0,0,0.65)] backdrop-blur-xl">
                        <div className="mb-3 flex items-start justify-between gap-3">
                            ...
                            <button onClick={() => setEnabledLayers((prev) => ({ ...prev, sigint: false }))} className="rounded-full border border-white/10 px-2 py-1 text-xs text-stone-400 hover:border-cyan-300/40 hover:text-cyan-200">
                                close
                            </button>
                        </div>

                        <div className="mb-3 grid grid-cols-4 gap-1 rounded-xl border border-white/5 bg-black/35 p-1">
                            {["overview", "navigation", "transponder", "space"].map((tab) => (<button key={tab} onClick={() => setSigintFilters((prev) => ({ ...prev, tab }))} className={`rounded-lg px-2 py-1.5 text-[0.58rem] font-bold uppercase tracking-wider transition ${sigintFilters.tab === tab ? "bg-cyan-400/20 text-cyan-100" : "text-stone-500 hover:text-stone-200"}`}>
                                    {tab}
                                </button>))}
                        </div>
                ...
                ["xpdr", sigintData?.stats.transponder ?? 0],
            ].map(([label, value]) => (<div key={label} className="rounded-xl border border-white/5 bg-black/35 p-2">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                            {["low", "medium", "high", "critical"].map((severity) => (<button key={severity} onClick={() => setSigintFilters((prev) => ({ ...prev, severity }))} className={`rounded-full border px-2.5 py-1 text-[0.6rem] font-bold uppercase tracking-wider ${sigintFilters.severity === severity ? "border-cyan-300/60 bg-cyan-400/15 text-cyan-100" : "border-white/10 text-stone-500 hover:text-stone-200"}`}>
                                    {severity}
                                </button>))}
                        </div>

                        <div className="mt-3 grid grid-cols-3 gap-2 text-[0.65rem] text-stone-300">
                            {["cells", "points", "labels"].map((key) => (<label key={key} className="flex items-center justify-center gap-2 rounded-xl border border-white/5 bg-black/25 px-2 py-2">
                                    <input type="checkbox" checked={sigintFilters[key]} onChange={() => setSigintFilters((prev) => ({ ...prev, [key]: !prev[key] }))}/>
                                    {key}
                                </label>))}
                        </div>
                        ...
                        <div className="mt-3 max-h-32 overflow-auto rounded-xl border border-white/5 bg-black/25 p-2">
                            {(sigintData?.sources || []).slice(0, 8).map((src) => (<div key={src.id || src.label} className="flex items-center justify-between gap-2 border-b border-white/5 py-1.5 last:border-0">
                ...
                {enabledLayers.recon === true && (<ReconModePanel_1.ReconModePanel onClose={() => setEnabledLayers((prev) => ({ ...prev, recon: false }))} onFoundCount={setReconCount} onLocations={setReconLocations} stacked={enabledLayers.sigint === true}/>)}
            ...
            return (<section key={group.id} className="rounded-2xl border border-white/5 bg-stone-950/45 p-2.5">
                    ...
                    return (<div key={layer.id} className={`flex items-center justify-between gap-3 rounded-xl border px-3 py-2.5 transition-all duration-200 ${checked ? "border-white/10 bg-white/[0.055]" : "border-stone-900 bg-black/20 hover:border-stone-700/70 hover:bg-stone-900/40"}`}>
                                            ...
                                            {group.id === "airspace" && enabledLayers.radar === true && (<div className="mt-2 rounded-xl border border-cyan-400/10 bg-black/25 p-2">
                        ...
                        return (<button key={filter.id} onClick={() => setRadarFilter(filter.id)} className={`rounded-lg border px-2.5 py-2 text-left transition-all duration-150 ${active ? "border-emerald-300/40 bg-emerald-400/10 text-stone-100 shadow-[0_0_24px_rgba(16,185,129,0.08)]" : "border-stone-900 bg-black/20 text-stone-500 hover:border-stone-700 hover:text-stone-300"}`}>
                ...
                <nav className="fixed right-1/2 top-4 z-30 translate-x-1/2">
                    <div className="flex min-w-max items-center gap-2 overflow-x-auto rounded-full border border-stone-900/80 bg-stone-950/80 p-1.5 text-sm backdrop-blur-sm transition-all">
                        {isSearchOpen ? (<form onSubmit={handleSearch} className="flex items-center gap-2 px-3 bg-stone-900 rounded-full">
                                ...
                                <input autoFocus type="text" placeholder="Search location, coordinate, etc..." className="bg-transparent text-white outline-none w-64 text-sm font-mono placeholder:text-stone-600 py-1.5" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onBlur={() => !searchQuery && setIsSearchOpen(false)}/>
                            </form>) : (<button onClick={() => setIsSearchOpen(true)} className="rounded-full p-2 transition-all duration-150 hover:bg-stone-900 hover:text-stone-200 text-stone-400">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-5">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"/>
                                </svg>
                            </button>)}
                        {TABS.map((t) => (<button key={t} onClick={() => applyTabPreset(t)} className={`whitespace-nowrap rounded-full px-5 py-1.5 transition-all duration-150 ${activeTab === t ? "bg-stone-800 text-white" : "text-stone-500 hover:bg-stone-900 hover:text-stone-200"}`}>
                                {t}
                            </button>))}
                    </div>
                </nav>
                ...
                <nav className="fixed bottom-4 right-1/2 z-30 flex translate-x-1/2 items-center gap-4">
                    {showImageryPicker && (<div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 flex items-end gap-1.5 rounded-2xl border border-stone-800/80 bg-stone-950/95 p-2 backdrop-blur-md shadow-2xl">
                            {IMAGERY_CATALOG.map((layer) => {
                const active = activeImagery === layer.id;
                return (<button key={layer.id} onClick={() => {
                        setActiveImagery(layer.id);
                        setShowImageryPicker(false);
                    }} className={`flex flex-col items-center gap-1 p-1.5 rounded-xl transition-all duration-150 ${active ? "ring-1 ring-stone-400 bg-stone-800/60" : "hover:bg-stone-900/80"}`} title={layer.sub}>
                    ...
                    <div className="flex min-w-max items-center gap-2 overflow-x-auto rounded-full border border-stone-900/80 bg-stone-950/80 p-1.5 text-sm backdrop-blur-sm">
                        ...
                        <div className="w-px h-4 bg-stone-800 mx-1"></div>
                        {STYLE_OPTIONS.map((s) => (<button key={s.id} onClick={() => setActiveStyle(s.id)} className={`rounded-full px-3 py-1.5 font-mono text-xs transition-all ${activeStyle === s.id ? "bg-cyan-400/20 text-cyan-300" : "text-stone-400 hover:bg-stone-900 hover:text-stone-200"}`}>
                                {s.label}
                            </button>))}
                    </div>
                    <button onClick={() => setShowImageryPicker((v) => !v)} className={`rounded-full border p-2 transition-all duration-150 ${showImageryPicker ? "border-amber-500/50 bg-amber-500/15 text-amber-300" : "border-stone-900 text-stone-400 hover:bg-stone-900 hover:text-stone-200"}`} title={`Basemap: ${IMAGERY_CATALOG.find((l) => l.id === activeImagery)?.label ?? activeImagery}`}>
                ...
                <nav className="fixed bottom-4 right-4 z-30 flex flex-col items-end gap-2">
                    ...
                    <button onClick={() => setStreetViewMode((v) => !v)} className={`flex min-w-max items-center gap-2 overflow-x-auto rounded-full border p-1.5 px-3 text-sm backdrop-blur-sm transition ${streetViewMode ? "border-sky-500/70 bg-sky-900/45 text-sky-200" : "border-stone-900/80 bg-stone-950/80 text-stone-300 hover:bg-stone-900/80"}`} title={streetViewMode ? "KartaView street imagery overlay active" : "Enable KartaView street imagery overlay"}>
                ...
                <nav className="fixed right-4 top-4 z-30 flex flex-col items-end gap-2">
                    ...
                    <WorldMonitorDeck_1.default visible={enabledLayers.gil === true} layers={worldMonitorLayers} onToggle={handleToggleWmLayer} counts={wmCounts} has_correlations={geoIntelData ? geoIntelData.correlations.length > 0 : false}/>
                </nav>

                <div className="pointer-events-none absolute right-4 top-24 z-40 flex flex-col items-end space-y-4">
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefFlight}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefFlight}>
                            <FlightDetailsSidebar_1.default flight={selectedFlight} onClose={() => setSelectedFlight(null)}/>
                        </div>
                    </react_draggable_1.default>
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefAirport}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefAirport}>
                            <AirportDetailsSidebar_1.default airport={selectedAirport} onClose={() => setSelectedAirport(null)}/>
                        </div>
                    </react_draggable_1.default>
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefSatellite}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefSatellite}>
                            <SatelliteDetailsSidebar_1.default satellite={selectedSatellite} onClose={() => setSelectedSatellite(null)}/>
                        </div>
                    </react_draggable_1.default>
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefQuake}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefQuake}>
                            <QuakeDetailsSidebar_1.default quake={selectedQuake} onClose={() => setSelectedQuake(null)}/>
                        </div>
                    </react_draggable_1.default>
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefRadio}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefRadio}>
                            <RadioDetailsSidebar_1.default radio={selectedRadio} onClose={() => setSelectedRadio(null)}/>
                        </div>
                    </react_draggable_1.default>
                    {selectedCamera && (<CameraDetailsSidebar_1.default camera={selectedCamera} onClose={() => setSelectedCamera(null)}/>)}
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefIntel}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefIntel}>
                            <IntelDetailsSidebar_1.default intel={selectedIntel} onClose={() => setSelectedIntel(null)}/>
                        </div>
                    </react_draggable_1.default>
                    {selectedSigint && (<react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefSigint}>
                            <div className="pointer-events-auto cursor-move" ref={nodeRefSigint}>
                                <div className="w-[26rem] rounded-2xl border border-cyan-400/20 bg-stone-950/90 p-4 text-stone-200 shadow-2xl backdrop-blur-xl">
                                    ...
                                    {selectedSigint.type === "cell" ? (<div className="space-y-3">
                                            ...
                                            <div className="max-h-28 overflow-auto rounded-xl border border-white/5 bg-black/30 p-2">
                                                {selectedSigint.data.evidence.map((ev) => (<div key={`${ev.label}-${ev.observed_at}`} className="border-b border-white/5 py-1.5 last:border-0">
                    ...
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefCity}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefCity}>
                            <CityDetailsSidebar_1.default city={selectedCity} onClose={() => setSelectedCity(null)} worldMonitorActive={worldMonitorActive} geoIntel={geoIntelData} onCameraSelect={setSelectedCamera}/>
                        </div>
                    </react_draggable_1.default>
                    <react_draggable_1.default handle=".cursor-move" nodeRef={nodeRefBuilding}>
                        <div className="pointer-events-auto cursor-move" ref={nodeRefBuilding}>
                            <BuildingDetailsSidebar_1.default buildingCoord={selectedBuilding} onClose={() => setSelectedBuilding(null)}/>
                        </div>
                    </react_draggable_1.default>
                </div>

                
                <CountryDashboard_1.default iso={selectedCountry} geoIntel={geoIntelData} onClose={() => setSelectedCountry(null)}/>
            </div>

            
            {enabledLayers.gil && geoIntelData && (<div className="fixed bottom-0 left-0 right-0 z-[100] w-full transition-transform duration-500 translate-y-[calc(100%-2rem)] hover:translate-y-0 group pointer-events-auto">
                    ...
                    <div className="bg-[#0a0a0a] border-t border-stone-800 shadow-[0_-16px_48px_rgba(0,0,0,0.9)] max-h-[80vh] overflow-y-auto">
                        <BottomIntelligenceDeck_1.BottomIntelligenceDeck data={geoIntelData} loading={geoIntelLoading}/>
                    </div>
                </div>)}
